...
!
      subroutine e_c3d_se2(co,kon,lakonl,p1,p2,omx,bodyfx,nbody,s,sm,&
      ...
      !     sensitivity of the element matrix multiplied by the
      ...
      !
      coef=(/1.d0,-1.d0,-1.d0,1.d0/)
      coef1=(/1.d0,1.d0,-1.d0,-1.d0/)
      coef2=(/1.d0,-1.d0,1.d0,-1.d0/)
      !
      do i=1,4
         coef(i)=coef(i)/(4.d0*distmin**2)
      enddo
      !
      tvar(1)=time
      tvar(2)=ttime+time
      ...
      !
      do i=1,3
         xl(i,iactive1)=xl(i,iactive1)+coef1(ij)*xdesi(i,idesvar1)
         xl(i,iactive2)=xl(i,iactive2)+coef2(ij)*xdesi(i,idesvar2)
      enddo
      !
          ...
          if((nope.eq.20).or.(nope.eq.8).or.&
             ...
             do i=1,nopes
                ...
                if(node.eq.nodedesi(idesvar1)) then
                   do j=1,3
                      xl2(j,i)=&
                           xl2(j,i)+coef1(ij)*xdesi(j,idesvar1)
                   enddo
                   exit
                endif
             enddo
             do i=1,nopes
                ...
                if(node.eq.nodedesi(idesvar2)) then
                   do j=1,3
                      xl2(j,i)=&
                           xl2(j,i)+coef2(ij)*xdesi(j,idesvar2)
                   enddo
                   exit
                endif
             enddo
      ...
      if(sigma.le.0.d0) then
         ...
         if((ieigenfrequency.ne.1).and.(iperturb(2).eq.1)) then
               ...
               if((rhsi.eq.1).and.(idist.eq.1)) then
                  do i=1,3*nope
                     dfl2(idesloc1,idesloc2,i)=&
                        coef(ij)*ff(i)+dfl2(idesloc1,idesloc2,i)
                  enddo
               endif
         endif
      !
