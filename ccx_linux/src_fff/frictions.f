...
!
      ...
      !
      !       if(nelcon(1,imat).gt.0) nelcon(1,imat)=max(nelcon(1,imat),7)
      !
      ...
      do
         ...
         do i=1,2
            read(textpart(i)(1:20),'(f20.0)',iostat=istat)&
                 elcon(5+i,1,imat)
            if(istat.gt.0) then
               call inputerror(inpc,ipoinpc,iline, "*FRICTION%",ier)                    "*FRICTION%",ier)
               return
            endif
         enddo
         ...
         if(elcon(7,1,imat).le.0.d0) then
            ...
            write(*,*) '         a experimentally based stick slope'             '         a experimentally based stick slope'
            call inputwarning(inpc,ipoinpc,iline, "*FRICTION%")      "*FRICTION%")
