...
!
      ...
      loop: do i=2,n
         if(textpart(i)(1:5).eq.'NSET=') then
               ...
               if(set(js).eq.noset) then
                  ...
                  if(iendset(js).eq.nalset) then
                     ...
                     if(nalset+nn.gt.nalset_) then
                        write(*,*) '*ERROR reading *NODE: increase nalset_'                         '*ERROR reading *NODE: increase nalset_'
                        ier=1
                        return
                     endif
                     do k=1,nn
            ...
            write(*,*) '*WARNING reading *NODE: parameter not recognized:'              '*WARNING reading *NODE: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*NODE%")      "*NODE%")
         endif
      enddo loop
      !
      do
         ...
         if(istat.gt.0) then
            call inputerror(inpc,ipoinpc,iline, "*NODE%",ier)                 "*NODE%",ier)
            return
         endif
         if(n.eq.1) then
            co(1,i)=0.d0
