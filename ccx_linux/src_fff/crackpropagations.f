...
!
      ...
      do i=2,n
         ...
         elseif(textpart(i)(1:6).eq.'INPUT=') then
            ...
            write(*,*) '*WARNING reading *CRACK PROPAGATION: parameter not recognized:'       '*WARNING reading *CRACK PROPAGATION: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*CRACKPROPAGATION%")      "*CRACKPROPAGATION%")
         endif
      enddo
      !
      ...
      elseif(solver(1:6).eq.'PASTIX') then
         ...
         write(*,*) '*WARNING reading *CRACK PROPAGATION: unknown solver;'           '*WARNING reading *CRACK PROPAGATION: unknown solver;'
      ...
      if(istat.gt.0) then
         call inputerror(inpc,ipoinpc,iline, "*CRACKPROPAGATION%",ier)              "*CRACKPROPAGATION%",ier)
         return
      endif
      ...
      if(tper.lt.0.d0) then
         write(*,*) '*ERROR reading *CRACK PROPAGATION: step size is negative'        '*ERROR reading *CRACK PROPAGATION: step size is negative'
         ier=1
         return
      ...
      if(idrct.ne.1) then
         if(dabs(tmin).lt.1.d-6*tper) then
            ...
            write(*,*) '         increment time and 1.e-6 times the step time'               '         increment time and 1.e-6 times the step time'
