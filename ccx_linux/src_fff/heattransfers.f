...
!
      ...
      elseif((iperturb(1).eq.1).and.(istep.gt.1)) then
         write(*,*) '*ERROR reading *HEAT TRANSFER: perturbation analysis is'           '*ERROR reading *HEAT TRANSFER: perturbation analysis is'
      ...
      !
      if(istep.lt.1) then
         write(*,*) '*ERROR reading *HEAT TRANSFER: *HEAT TRANSFER can only'           '*ERROR reading *HEAT TRANSFER: *HEAT TRANSFER can only'
      ...
      do i=2,n
         ...
         elseif(textpart(i)(1:17).eq.'TOTALTIMEATSTART=') then
            ...
            write(*,*) '*WARNING reading *HEAT TRANSFER: parameter not recognized:'            '*WARNING reading *HEAT TRANSFER: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*HEAT TRANSFER%")      "*HEAT TRANSFER%")
         endif
      enddo
      ...
      if((ithermal(1).eq.0).and.(nmethod.ne.1).and.&
         ...
         write(*,*) '*ERROR reading *HEAT TRANSFER: please define initial '           '*ERROR reading *HEAT TRANSFER: please define initial '
      ...
      if((nmethod.ne.2).and.(iperturb(1).ne.0)) then
         ...
         elseif(solver(1:6).eq.'PASTIX') then
            ...
            write(*,*) '*WARNING reading *HEAT TRANSFER: unknown solver;'                '*WARNING reading *HEAT TRANSFER: unknown solver;'
         ...
         if((istat.lt.0).or.(key.eq.1)) then
            if(iperturb(1).ge.2) then
               write(*,*) '*WARNING reading *HEAT TRANSFER: a nonlinear geomet                    '*WARNING reading *HEAT TRANSFER: a nonlinear geomet&
      ...
      ecified'
         ...
         if(istat.gt.0) then
            call inputerror(inpc,ipoinpc,iline, "*HEAT TRANSFER%",ier)                 "*HEAT TRANSFER%",ier)
            return
         endif
         ...
         if(tinc.le.0.d0) then
            write(*,*) '*ERROR reading *HEAT TRANSFER: initial increment size                  '*ERROR reading *HEAT TRANSFER: initial increment size&
      is negative'
         endif
         if(tper.le.0.d0) then
            write(*,*) '*ERROR reading *HEAT TRANSFER: step size is negative'              '*ERROR reading *HEAT TRANSFER: step size is negative'
         endif
      ...
      elseif(nmethod.eq.2) then
         ...
         call getnewline(inpc,textpart,istat,n,key,iline,ipol,inl,&
              ipoinp,inp,ipoinpc)
         if((istat.lt.0).or.(key.eq.1)) then
            write(*,*) '*ERROR reading *HEAT TRANSFER: definition not complete'             '*ERROR reading *HEAT TRANSFER: definition not complete'
            write(*,*) '  '
            call inputerror(inpc,ipoinpc,iline, "*HEAT TRANSFER%",ier)                 "*HEAT TRANSFER%",ier)
            return
         endif
