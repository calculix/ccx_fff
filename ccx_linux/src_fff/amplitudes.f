...
!
      ...
      do i=2,n
         ...
         elseif(textpart(i)(1:6).eq.'SHIFTX') then
               ...
               if(istat.gt.0) then
                  call inputerror(inpc,ipoinpc,iline, "*AMPLITUDE%",ier)                       "*AMPLITUDE%",ier)
                  return
               endif
         elseif(textpart(i)(1:6).eq.'SHIFTY') then
            ...
            write(*,*) '*WARNING reading *AMPLITUDE: parameter not recognized:'              '*WARNING reading *AMPLITUDE: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*AMPLITUDE%")      "*AMPLITUDE%")
         endif
      enddo
      !
         ...
         do i=1,4
            if(textpart(2*i-1)(1:1).ne.' ') then  
               ...
               if(namtot.gt.namtot_) then
                  write(*,*) '*ERROR reading *AMPLITUDE: increase namtot_'                     '*ERROR reading *AMPLITUDE: increase namtot_'
                  ier=1
                  return
               endif
      ...
      if(namta(1,nam).gt.namta(2,nam)) then
         ...
         write(*,*) '*WARNING reading *AMPLITUDE: *AMPLITUDE definition ',            '*WARNING reading *AMPLITUDE: *AMPLITUDE definition ',&
             amname(nam)(1:ipos-1)
