...
!
      ...
      do m=2,n
         if(textpart(m)(1:5).eq.'TYPE=') then
            ...
            if(textpart(m)(19:19).ne.' ') then
               ...
               call inputwarning(inpc,ipoinpc,iline, "*NETWORK MPC%")      "*NETWORK MPC%")
             endif
         else
             write(*,*) '*WARNING reading *NETWORK MPC: parameter not recognized:'              '*WARNING reading *NETWORK MPC: parameter not recognized:'
             write(*,*) '         ',&
      ...
      !
      if(istep.gt.0) then
         write(*,*) '*ERROR reading *NETWORK MPC: *NETWORK MPC should be placed'           '*ERROR reading *NETWORK MPC: *NETWORK MPC should be placed'
      ...
      do
         ...
         do
            ...
            if((istat.lt.0).or.(key.eq.1)) then
               write(*,*) '*ERROR reading *NETWORK MPC: mpc definition ',nmpc                 '*ERROR reading *NETWORK MPC: mpc definition ',nmpc
               write(*,*) '  is not complete. '
               call inputerror(inpc,ipoinpc,iline, "*NETWORK MPC%",ier)                    "*NETWORK MPC%",ier)
               return
            endif
            !
            do i=1,n/3
               ...
               if(mpcfree.eq.0) then
                  write(*,*) '*ERROR reading *NETWORK MPC: increase memmpc_'                        '*ERROR reading *NETWORK MPC: increase memmpc_'
                  ier=1
                  return
               endif
               !
