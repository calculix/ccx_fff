      ...
      do i=2,n
         if(textpart(i)(1:5).eq.'NAME=')&
            ...
            if(textpart(i)(86:86).ne.' ') then
               write(*,*) '*ERROR reading *SURFACE: surface name too long'                 '*ERROR reading *SURFACE: surface name too long'
         ...
         elseif(textpart(i)(1:5).eq.'TYPE=') then
            ...
            elseif(textpart(i)(6:9).eq.'NODE') then
               ...
               write(*,*) '*ERROR reading *SURFACE: unknown surface type'                   '*ERROR reading *SURFACE: unknown surface type'
               ier=1
               return
            endif
         else
            write(*,*) '*WARNING reading *SURFACE: parameter not recognized:'              '*WARNING reading *SURFACE: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*SURFACE%")      "*SURFACE%")
         endif
      enddo
      !
      ipos=index(noelset,' ')
      ...
      if(type.eq.'S') then
         ...
         do
            ...
            if(n.gt.1) then
               ...
               write(*,*) '       line allowed'
               call inputerror(inpc,ipoinpc,iline, "*SURFACE%",ier)                    "*SURFACE%",ier)
               return
            endif
            !
                     ...
                     do j=istartset(i),iendset(i)
                        if(ialset(j).gt.0) then
                           ...
                           if(nalset.gt.nalset_) then
                              write(*,*) '*ERROR reading *SURFACE: increase nalset_'                             '*ERROR reading *SURFACE: increase nalset_'
                              ier=1
                              return
                           endif
