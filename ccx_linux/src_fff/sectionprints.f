...
!
      ...
      do ii=2,n
        if(textpart(ii)(1:8).eq.'SURFACE=') then
          ...
          enddo
          if(i.gt.nset) then
             write(*,*) '*WARNING reading *SECTION PRINT: element surface ',                 '*WARNING reading *SECTION PRINT: element surface ',&
        ...
        elseif(textpart(ii)(1:11).eq.'FREQUENCYF=') then
           ...
           if(istat.gt.0) then
              call inputerror(inpc,ipoinpc,iline, "*SECTION PRINT%",ier)                   "*SECTION PRINT%",ier)
              return
           endif
           if(joutl.eq.0) then
              do
        ...
        elseif(textpart(ii)(1:11).eq.'TIMEPOINTS=') then
           ...
           if(i.gt.nam) then
              ...
              write(*,*) '*ERROR reading *SECTION PRINT: time points definition '                '*ERROR reading *SECTION PRINT: time points definition '&
                     ,timepointsname(1:ipos-1),' is unknown or empty'
              ier=1
              return
           endif
           if(idrct.eq.1) then
              write(*,*) '*ERROR reading *SECTION PRINT: the DIRECT option'                  '*ERROR reading *SECTION PRINT: the DIRECT option'
        ...
        elseif(textpart(ii)(1:5).eq.'NAME=') then
            ...
            write(*,*) '*WARNING reading *SECTION PRINT: parameter not recognized:'            '*WARNING reading *SECTION PRINT: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(ii)(1:index(textpart(ii),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*SECTION PRINT%")      "*SECTION PRINT%")
        endif
      enddo
      !
      if(name(1:1).eq.' ') then
         write(*,*) '*ERROR reading *SECTION PRINT: no NAME given'              '*ERROR reading *SECTION PRINT: no NAME given'
         write(*,*) '  '
         call inputerror(inpc,ipoinpc,iline, "*SECTION PRINT%",ier)              "*SECTION PRINT%",ier)
         return
      endif
      ...
      if(noset(1:1).eq.' ') then
         write(*,*) '*WARNING reading *SECTION PRINT: no set was defined'             '*WARNING reading *SECTION PRINT: no set was defined'
         call getnewline(inpc,textpart,istat,n,key,iline,ipol,inl,&
              ipoinp,inp,ipoinpc)
         return
      endif
      ...
      do
         ...
         loop: do ii=1,n
            if((textpart(ii)(1:4).ne.'DRAG').and.&
               ...
               write(*,*) '*WARNING reading *SECTION PRINT: label not applicable'                 '*WARNING reading *SECTION PRINT: label not applicable'
               write(*,*) '         or unknown; '
               call inputwarning(inpc,ipoinpc,iline, "*SECTION PRINT%")      "*SECTION PRINT%")
               cycle
            endif
            if((nef.eq.0).and.(textpart(ii)(1:4).eq.'DRAG')) then
               write(*,*) '*WARNING reading *SECTION PRINT: DRAG only makes '                 '*WARNING reading *SECTION PRINT: DRAG only makes '
            ...
            if(nprint.gt.nprint_) then
               write(*,*) '*ERROR reading *SECTION PRINT: increase nprint_'                  '*ERROR reading *SECTION PRINT: increase nprint_'
               ier=1
               return
            endif
            prset(nprint)=noset
