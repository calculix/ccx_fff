...
!
      ...
      if(network.gt.2) then
         ...
         !
         if(pressmin.lt.0.d0) then
            write(*,*) '*ERROR in initialchannel: minimum initial pressure'              '*ERROR in initialchannel: minimum initial pressure'
      ...
      do i=1,ntg
         ...
         if (v(0,node)-physcon(1).lt.1.d-10) then
            write(*,*) '*WARNING in initialchannel : the initial temperature f                 '*WARNING in initialchannel : the initial temperature f&
      or node',node
            write(*,*) 'is O Kelvin or less; the default is taken (293 K)'                 'is O Kelvin or less; the default is taken (293 K)'
            write(*,*)
      ...
      if(network.gt.2) then
         ...
         if(dabs(xflow).gt.1.d-30) then
            ...
            do j=1,nflow
               ...
               if(.not.gravity) then
                  write(*,*) '*ERROR in initialchannel: no gravity vector'                    '*ERROR in initialchannel: no gravity vector'
