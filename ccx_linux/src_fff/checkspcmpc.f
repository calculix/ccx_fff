          ...
          do l=nslavnode(i)+1,nslavnode(i+1)
            ...
            do j=nslavspc(1,l)+1,nslavspc(2,l)
              ...
              if((sp.lt.-0.08).or.(sp.gt.0.08))then
                if(debug)then
                  write(*,*) 'checkspcmpc: normal direction ', 'cannot be blocked'                       'cannot be blocked'
              ...
              if(islavmpc(2,j).ne.1)then
                ...
                if(id>0)then
                  if(islavnode(nslavnode(i)+id).ne.zs(dirdep))then
                    if(debug)then
                      write(*,*) 'checkspcmpc: only zyclic ', 'symmetry on slave nodes supported'                           'symmetry on slave nodes supported'
                      write(*,*) 'node',node, 'has no LM'
                    endif
                    incompatible=.true.
                    islavmpc(2,j)=-2
                ...
                if(index.ne.0) then
                  do
                    if(nodempc(1,index).ne.zs(dirdep))then
                      if(debug)then
                        write(*,*) 'checkspcmpc: only zyclicy', 'symmetry and directional',                             'symmetry and directional', ' blocking supported'                             ' blocking supported'
                        write(*,*) 'node',node, 'has no LM'
                      endif
                      incompatible=.true.
                      islavmpc(2,j)=-2
                  ...
                  do
                    ...
                    if(id>0)then
                      ...
                      if(islavnode(nslavnode(i)+id).eq.&
                        ...
                        if(debug)then
                          write(*,*) 'checkspcmpc:mpc with', 'slave node ruin diagonal Dd!'                               'slave node ruin diagonal Dd!'
                          write(*,*) 'node',nodempc(1,index), 'has no LM'                               'has no LM'
                        endif
                        islavmpc(2,j)=-2
                        islavact(nslavnode(i)+id)=-2
                        incompatible=.true.
            ...
            if(.not.((zs(1).eq.zs(2)).or.(zs(2).eq.zs(3))&
              ...
              if(debug)then
                write(*,*) 'checkspcmpc: only zyclic symmetry', 'and directional blocking supported'                     'and directional blocking supported'
                write(*,*) 'node',node, 'has no LM'
                ...
                if(j.ne.i)then
                  ...
                  if(id>0)then
                    if(islavnode(nslavnode(j)+id).eq.node)then
                      islavact(l)=-2
                      write(*,*)'checkspcmpc: node',node, 'tie1s',i,'tie2s',j                           'tie1s',i,'tie2s',j
                      write(*,*)'in more than one contact', 'tie and set NoLM!'                           'tie and set NoLM!'
                    endif
                  endif                   
                  ...
                  if(id>0)then
                    if(imastnode(nmastnode(j)+id).eq.node)then
                      islavact(l)=-2
                      write(*,*)'checkspcmpc: node',node, 'tie1s',i,'tie2m',j                           'tie1s',i,'tie2m',j
                      write(*,*)'in more than one', ' contact tie and set NoLM!'                           ' contact tie and set NoLM!'
                    endif
                  endif                   
                endif
              enddo
