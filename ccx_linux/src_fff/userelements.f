...
!
      ...
      do i=2,n
         ...
         !             read(textpart(i)(7:16),'(i10)',iostat=istat) number
         !             if(istat.gt.0) then
         ...
         elseif(textpart(i)(1:6).eq.'NODES=') then
            ...
            if(istat.gt.0) then
               call inputerror(inpc,ipoinpc,iline, "*USER ELEMENT%",ier)                               "*USER ELEMENT%",ier)
               return
            endif
      ...
      !          write(*,*) '       element number ',number,' exceeds 9999'
      !          ier=1
      !          return
      !       endif
      !
      if(intpoints.gt.255) then
         write(*,*) '*ERROR reading *USER ELEMENT'
         write(*,*) '       number of integration points ',intpoints, ' exceeds 255'             ' exceeds 255'
         ier=1
         return
      endif
      !
      if(maxdof.gt.255) then
         write(*,*) '*ERROR reading *USER ELEMENT'
         write(*,*) '       highest degree of freedom ',maxdof, ' exceeds 255'             ' exceeds 255'
         ier=1
         return
      endif
      !
