...
!
      ...
      !
      if(istep.lt.1) then
         write(*,*) '*ERROR reading *NODE PRINT: *NODE PRINT should only be'              '*ERROR reading *NODE PRINT: *NODE PRINT should only be'
      ...
      do ii=2,n
        ...
        elseif(textpart(ii)(1:10).eq.'FREQUENCY=') then
           ...
           if(istat.gt.0) then
              call inputerror(inpc,ipoinpc,iline, "*NODE PRINT%",ier)                   "*NODE PRINT%",ier)
              return
           endif
           if(joutl.eq.0) then
              do
        ...
        elseif(textpart(ii)(1:11).eq.'TIMEPOINTS=') then
           ...
           if(i.gt.nam) then
              ...
              write(*,*) '*ERROR reading *NODE PRINT: time points definition '                '*ERROR reading *NODE PRINT: time points definition '&
           ...
           jout(2)=1
            ...
            write(*,*) '*WARNING in modaldynamics: parameter not recognized:'              '*WARNING in modaldynamics: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(ii)(1:index(textpart(ii),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*NODE PRINT%")      "*NODE PRINT%")
        endif
      enddo
      !
      ...
      do
         ...
         do ii=1,n
            if((textpart(ii)(1:4).ne.'U   ').and.&
               ...
               write(*,*) '*WARNING reading *NODE PRINT: label not applicable'                  '*WARNING reading *NODE PRINT: label not applicable'
               write(*,*) '         or unknown; '
               call inputwarning(inpc,ipoinpc,iline, "*NODE PRINT%")      "*NODE PRINT%")
               cycle
            endif
            if(textpart(ii)(1:4).eq.'RFL ') then
               if(ithermal(1).lt.2) then
                  write(*,*) '*WARNING reading *NODE PRINT: RFL only makes '                    '*WARNING reading *NODE PRINT: RFL only makes '
            ...
            elseif((textpart(ii)(1:4).eq.'VF  ').or.&
               ...
               if(nef.eq.0) then
                  write(*,*) '*WARNING reading *NODE PRINT: VF, PSF, TSF,'                     '*WARNING reading *NODE PRINT: VF, PSF, TSF,'
