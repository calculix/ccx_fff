...
!
      ...
      do i=2,n
         if(textpart(i)(1:6).eq.'ELSET=') then
            ...
            write(*,*) '*WARNING reading *GAP: parameter not recognized:'              '*WARNING reading *GAP: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*GAP%")      "*GAP%")
         endif
      enddo
      !
      !     6 parameters
      ...
      do
         ...
         do i=1,min(4,n)
            read(textpart(i)(1:20),'(f20.0)',iostat=istat)&
                 elcon(i,ntmat,nmat)
            if(istat.gt.0) then
               call inputerror(inpc,ipoinpc,iline, "*GAP%",ier)                               "*GAP%",ier)
               return
            endif
         enddo
         !
