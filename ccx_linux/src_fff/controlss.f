...
!
      ...
      do i=2,n
         if(textpart(i)(1:5).eq.'RESET') then
            ...
            ctrl(57)=60.5d0
            write(*,*)
            write(*,*) '*INFO: control parameters reset to default'               '*INFO: control parameters reset to default'
            exit
         !
         elseif(textpart(i)(1:29).eq.'PARAMETERS=TIMEINCREMENTATION')&
            ...
            do j=1,min(8,n)
               ...
               if(istat.gt.0) then
                  call inputerror(inpc,ipoinpc,iline, "*CONTROLS%",ier)                       "*CONTROLS%",ier)
                  return
               endif
               ctrl(j)=dble(k)+0.5d0
            enddo
         ...
         elseif(textpart(i)(1:18).eq.'PARAMETERS=CONTACT') then
            ...
            if((ctrl(55).lt.0.d0).or.(ctrl(55).gt.1.d0)) then
               write(*,*) '*ERROR reading *CONTROLS'
               write(*,*) '       alea should belong to the interval [0.,1.]'              '       alea should belong to the interval [0.,1.]'
               call inputerror(inpc,ipoinpc,iline, "*CONTROLS%",ier)                    "*CONTROLS%",ier)
            endif
            !
            ...
            write(*,*) '*WARNING in controlss: parameter not recognized:'              '*WARNING in controlss: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*CONTROLS%")      "*CONTROLS%")
         endif
      enddo
      !
      call getnewline(inpc,textpart,istat,n,key,iline,ipol,inl,&
