...
!
      ...
      !
      if(nmat.eq.0) then
         write(*,*) '*ERROR reading *ELASTIC: *ELASTIC should be preceded'             '*ERROR reading *ELASTIC: *ELASTIC should be preceded'
      ...
      do i=2,n
         if(textpart(i)(1:5).eq.'TYPE=') then
            ...
            write(*,*) '*WARNING reading *ELASTIC: parameter not recognized:'              '*WARNING reading *ELASTIC: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*ELASTIC%")      "*ELASTIC%")
         endif
      enddo
      !
      nelcon(1,nmat)=ityp
      ...
      if(ityp.eq.2) then
         do
            ...
            if(n.lt.2) then
               ...
               call inputerror(inpc,ipoinpc,iline, "*ELASTIC%",ier)                    "*ELASTIC%",ier)
               return
            endif
            do i=1,2
      ...
      elseif(ityp.eq.9) then
         do
            ...
            if((istat.lt.0).or.(key.eq.1)) then
               write(*,*) '*ERROR reading *ELASTIC: orthotropic definition'                 '*ERROR reading *ELASTIC: orthotropic definition'
      ...
      elseif(ityp.eq.21) then
         do
            ...
            call getnewline(inpc,textpart,istat,n,key,iline,ipol,inl,&
                 ipoinp,inp,ipoinpc)
            if((istat.lt.0).or.(key.eq.1)) then
               write(*,*) '*ERROR reading *ELASTIC: anisotropic definition'                  '*ERROR reading *ELASTIC: anisotropic definition'
               write(*,*) '  is not complete. '
               call inputerror(inpc,ipoinpc,iline, "*ELASTIC%",ier)                    "*ELASTIC%",ier)
               return
            endif
