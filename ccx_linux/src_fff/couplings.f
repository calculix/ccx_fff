...
!
      ...
      do i=2,n
         if(textpart(i)(1:8).eq.'REFNODE=') then
            ...
            if(istat.gt.0) then
               call inputerror(inpc,ipoinpc,iline, "*COUPLING%",ier)                    "*COUPLING%",ier)
               return
            endif
         ...
         elseif(textpart(i)(1:15).eq.'CONSTRAINTNAME=') then
            ...
            write(*,*) '*WARNING reading *COUPLING: parameter not recognized:'                 '*WARNING reading *COUPLING: parameter not recognized:'
            write(*,*) '         ',&
                 textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*COUPLING%")      "*COUPLING%")
         endif
      enddo
      !
      if(name(1:1).eq.' ') then
         write(*,*) '*ERROR reading *COUPLING: no CONTRAINT NAME given'              '*ERROR reading *COUPLING: no CONTRAINT NAME given'
         write(*,*) '  '
         call inputerror(inpc,ipoinpc,iline, "*COUPLING%",ier)              "*COUPLING%",ier)
         return
      endif
      ...
      if(orientation.eq.'                    ') then
         ...
         enddo
         if(i.gt.norien) then
            write(*,*) '*ERROR reading *COUPLING: nonexistent orientation'             '*ERROR reading *COUPLING: nonexistent orientation'
            write(*,*) '  '
            call inputerror(inpc,ipoinpc,iline, "*COUPLING%",ier)                 "*COUPLING%",ier)
            return
         endif
      ...
      if(textpart(1)(2:10).eq.'KINEMATIC') then
         ...
         nk=nk+1
         if(nk.gt.nk_) then
            write(*,*) '*ERROR reading *KINEMATIC: increase nk_'                 '*ERROR reading *KINEMATIC: increase nk_'
            ier=1
            return
         endif
         irotnode_kin=nk
         ...
         do k=1,3
            ...
            if(nmpc.gt.nmpc_) then
               write(*,*) '*ERROR reading *KINEMATIC: increase nmpc_'                    '*ERROR reading *KINEMATIC: increase nmpc_'
               ier=1
               return
            endif
            !
         ...
         do
            ...
            if(istat.gt.0) then
               call inputerror(inpc,ipoinpc,iline, "*KINEMATIC%",ier)                    "*KINEMATIC%",ier)
               return
            endif
            ...
            if(textpart(2)(1:1).eq.' ') then
               ...
               if(istat.gt.0) then
                  call inputerror(inpc,ipoinpc,iline, "*BOUNDARY%",ier)                       "*BOUNDARY%",ier)
                  return
               endif
            endif
      ...
      elseif(textpart(1)(2:13).eq.'DISTRIBUTING') then
         ...
         do m=1,npt
            nk=nk+1
            if(nk.gt.nk_) then
               write(*,*) '*ERROR reading *DISTRIBUTING: increase nk_'                    '*ERROR reading *DISTRIBUTING: increase nk_'
               ier=1
               return
            endif
            node=ics(1,m)
            ...
            do k=1,3
               ...
               !                cgx(k)=cgx(k)+co(k,node)
               !
               nmpc=nmpc+1
               if(nmpc.gt.nmpc_) then
                  write(*,*) '*ERROR reading *DISTRIBUTING: increase nmpc_'                       '*ERROR reading *DISTRIBUTING: increase nmpc_'
                  ier=1
                  return
               endif
               !
         ...
         do m=1,npt
            node=ics(1,m)
            do k=1,3
               !                stdev=stdev+(dcs(m)*co(k,node)-cgx(k))**2
         ...
         do
            ...
            if(ibounend.gt.6) then
               ...
               write(*,*) '       exceed 6'
               write(*,*) '  '
               call inputerror(inpc,ipoinpc,iline, "*DISTRIBUTING%",ier)                    "*DISTRIBUTING%",ier)
               return
            elseif(ibounend.lt.ibounstart) then
               cycle
            endif
            ...
            do kk=ibounstart,ibounend
               ...
               do m=1,npt
                  do j=1,3
                     ...
                     mpcfree=nodempc(3,mpcfree)
                  enddo
               !                   write(*,*) 'couplings, co',m,(co(j,ics(1,m)),j=1,3)
               enddo
               nodempc(1,mpcfree)=irotnode(k)
               nodempc(2,mpcfree)=1
               coefmpc(mpcfree)=0.d0
               ...
               ikboun(id+1)=idof
               ilboun(id+1)=nboun
               !                write(*,*) 'couplings cgx',(cgx(l),l=1,3)
               !                write(*,*) 'couplings stdev',stdev
               !
               ...
               do
                  ...
                  do j=1,3
                     pi(j)=pi(j)-c1*aa(j)
                  enddo
                  !                   write(*,*) 'couplings c1 pi',c1,(pi(j),j=1,3)
                  !
                  c1=pi(1)*pi(1)+pi(2)*pi(2)+pi(3)*pi(3)
                  !                   if(c1.lt.1.d-20) then
                  if(c1.lt.stdev*1.d-10) then
                     write(*,*) '*WARNING reading *DISTRIBUTING: node ',node                          '*WARNING reading *DISTRIBUTING: node ',node
               ...
               do
                  ...
                  if(j.gt.3) j=1
                  nodempc(2,index2)=j
                  !                   write(*,*) 'couplings a',(coefmpc(index2))
               ...
               if(indexmax.eq.0) then
                  write(*,*) '*WARNING reading *DISTRIBUTING: no MPC is '                       '*WARNING reading *DISTRIBUTING: no MPC is '
      ...
      !          enddo
         ...
         write(*,*) '*ERROR reading *COUPLING: the line following'              '*ERROR reading *COUPLING: the line following'
