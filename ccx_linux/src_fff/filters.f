...
!
      ...
      do i=2,n
         ...
         !
            ...
            write(*,*) '*WARNING reading *FILTER: parameter not recognized:'              '*WARNING reading *FILTER: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*FILTER%")                 "*FILTER%")
         endif
      enddo 
      !
      !     reading the radii
      ...
      if((filteract.eq.1).or.(boundact.eq.1)) then
         ...
         if(filteract.eq.1) then
            ...
            if(istat.gt.0) then
               call inputerror(inpc,ipoinpc,iline, "*FILTER%",ier)                             "*FILTER%",ier)
               return
            endif
