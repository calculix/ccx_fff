...
!
      ...
      do i=2,n
         if(textpart(i)(1:4).eq.'DOF=') then
            ...
            if(istat.gt.0) then
               call inputerror(inpc,ipoinpc,iline, "*BASE MOTION%",ier)                    "*BASE MOTION%",ier)
               return
            endif
         ...
         elseif(textpart(i)(1:5).eq.'TYPE=') then
            ...
            write(*,*) '*WARNING reading *BASE MOTION: parameter not recognized:'              '*WARNING reading *BASE MOTION: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*BASE MOTION%")      "*BASE MOTION%")
         endif
      enddo
      !
      if(idof.eq.-1) then
