...
!
      ...
      do ii=2,n
         if(textpart(ii)(1:10).eq.'FREQUENCY=') then
            ...
            if(istat.gt.0) then
               call inputerror(inpc,ipoinpc,iline, "*CONTACT PRINT%",ier)                    "*CONTACT PRINT%",ier)
               return
            endif
            if(joutl.eq.0) then
               do
      ...
      finition '&
           ...
           if(idrct.eq.1) then
              write(*,*) '*ERROR reading *CONTACT PRINT: the DIRECT option'                  '*ERROR reading *CONTACT PRINT: the DIRECT option'
        ...
        elseif(textpart(ii)(1:7).eq.'MASTER=') then
           ...
           if(istat.gt.0) then
            ...
            write(*,*) '*WARNING reading *CONTACT PRINT: parameter not recognized:'            '*WARNING reading *CONTACT PRINT: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(ii)(1:index(textpart(ii),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*CONTACT PRINT%")      "*CONTACT PRINT%")
        endif
      enddo
      !
