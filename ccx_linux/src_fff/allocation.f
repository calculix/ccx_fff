...
!
      ...
      elseif(textpart(1)(1:10).eq.'*BOUNDARYF') then
        ...
        do
          ...
          if(istat.gt.0) then
            call inputerror(inpc,ipoinpc,iline, "*BOUNDARYF%",ier)                 "*BOUNDARYF%",ier)
            exit
          endif
          !
      ...
      elseif(textpart(1)(1:16).eq.'*CYCLICHARDENING') then
        ...
        do
          ...
          read(textpart(3)(1:20),'(f20.0)',iostat=istat)&
               temperature
          if(istat.gt.0) then
            call inputerror(inpc,ipoinpc,iline, "*CYCLIC HARDENING%",ier)                 "*CYCLIC HARDENING%",ier)
            exit
          endif
          if(ntmatl.eq.0) then
            npmatl=0
      ...
      elseif(textpart(1)(1:8).eq.'*DASHPOT') then
        ...
        call getnewline(inpc,textpart,istat,n,key,iline,ipol,inl,&
             ipoinp,inp,ipoinpc)
        if((istat.lt.0).or.(key.eq.1)) then
          call inputerror(inpc,ipoinpc,iline, "*DASHPOT%",ier)               "*DASHPOT%",ier)
          cycle
        endif
      ...
      elseif((textpart(1)(1:8).eq.'*DYNAMIC').or.&
             ...
             .or.&
             (textpart(1)(1:34).eq. '*UNCOUPLEDTEMPERATURE-DISPLACEMENT'))then             '*UNCOUPLEDTEMPERATURE-DISPLACEMENT'))then
        !
      ...
      elseif((textpart(1)(1:8).eq.'*ELEMENT').and.&
        ...
        elseif(textpart(i)(1:5).eq.'TYPE=') then
          ...
          if(label.eq.'        ') then
            write(*,*) '*ERROR in allocation: element type is lacking'                 '*ERROR in allocation: element type is lacking'
            write(*,*) '       '
            call inputerror(inpc,ipoinpc,iline, "*ELEMENT or *ELEMENT OUTPUT%",ier)                 "*ELEMENT or *ELEMENT OUTPUT%",ier)
            exit
          endif
          ...
          elseif(label(1:1).eq.'U') then
            ...
            if(nope.eq.-1) then
              ...
              write(*,*) '       ',label
              call inputerror(inpc,ipoinpc,iline, "*ELEMENT%",ier)                   "*ELEMENT%",ier)
              cycle loop
            endif
          endif
          ...
          if(igen) then
            ...
            do i=1,3
              read(textpart(i)(1:10),'(i10)',iostat=istat)&
                   ialset(i)
              if(istat.gt.0) then
                call inputerror(inpc,ipoinpc,iline, "*NSET or *ELSET%",ier)                     "*NSET or *ELSET%",ier)
                exit
              endif
            enddo
        ...
        do i=2,n
          if(textpart(i)(1:10).eq.'CONSTANTS=') then
            read(textpart(i)(11:20),'(i10)',iostat=istat)&
                 nconstants
            if(istat.gt.0) then
              call inputerror(inpc,ipoinpc,iline, "*FLUID SECTION%",ier)                   "*FLUID SECTION%",ier)
              exit
            endif
      ...
      elseif(textpart(1)(1:15).eq.'*GAPCONDUCTANCE') then
        ...
        do
          ...
          read(textpart(3)(1:20),'(f20.0)',iostat=istat)&
               temperature
          if(istat.gt.0) then
            call inputerror(inpc,ipoinpc,iline, "*GAP CONDUCTANCE%",ier)                 "*GAP CONDUCTANCE%",ier)
            exit
          endif
          if(ntmatl.eq.0) then
            npmatl=0
      ...
      elseif(textpart(1)(1:8).eq.'*HEADING') then
        if(nheading_.ne.0) then
          ...
          call inputerror(inpc,ipoinpc,iline, "*HEADING%",ier)               "*HEADING%",ier)
          cycle loop
        endif
        do
          call getnewline(inpc,textpart,istat,n,key,iline,ipol,inl,&
      ...
      elseif(textpart(1)(1:13).eq.'*MODALDAMPING') then
        if(textpart(2)(1:8).ne.'RAYLEIGH') then
          ...
          do
            ...
            read(textpart(1)(1:10),'(i10)',iostat=istat) i
            if(istat.gt.0) then
              call inputerror(inpc,ipoinpc,iline, "*MODAL DAMPING%",ier)                   "*MODAL DAMPING%",ier)
              exit
            endif
      ...
      do
        ...
        read(textpart(1)(1:10),'(i10)',iostat=istat) i
        if(istat.gt.0) then
          call inputerror(inpc,ipoinpc,iline, "*NODE or *NODE PRINT or *NODE FILE or *NODE OUTPUT%",               "*NODE or *NODE PRINT or *NODE FILE or *NODE OUTPUT%",&
               ier)
          exit
        endif
        nk_=max(nk_,i)
      ...
      elseif(textpart(1)(1:8).eq.'*PLASTIC') then
        ...
        do
          ...
          read(textpart(3)(1:20),'(f20.0)',iostat=istat)&
               temperature
          if(istat.gt.0) then
            call inputerror(inpc,ipoinpc,iline, "*PLASTIC%",ier)                 "*PLASTIC%",ier)
            exit
          endif
          if(ntmatl.eq.0) then
            npmatl=0
        ...
        do i=1,n
          ...
          if(textpart(i)(1:5).eq.'STEP=') then
            read(textpart(i)(6:15),'(i10)',iostat=istat)&
                 irestartstep
            if(istat.gt.0) then
              call inputerror(inpc,ipoinpc,iline, "*RESTART%",ier)                   "*RESTART%",ier)
              exit
            endif
          endif
        enddo
        ...
        if(lin) then
          ...
          do
            ...
            read(textpart(3)(1:20),'(f20.0)',iostat=istat)&
                 temperature
            if(istat.gt.0) then
              call inputerror(inpc,ipoinpc,iline, "*SPRING%",ier)                   "*SPRING%",ier)
              exit
            endif
            if(ntmatl.eq.0) then
              npmatl=0
        ...
        if(tabular) then
          ...
          do
            ...
            read(textpart(3)(1:20),'(f20.0)',iostat=istat)&
                 temperature
            if(istat.gt.0) then
              call inputerror(inpc,ipoinpc,iline, "*SURFACE BEHAVIOR%",ier)                   "*SURFACE BEHAVIOR%",ier)
              exit
            endif
            if(ntmatl.eq.0) then
              npmatl=0
      ...
      elseif(textpart(1)(1:4).eq.'*TIE') then
        ...
        if((islavset.ne.0).and.(imastset.ne.0)) then
          ...
          call inputerror(inpc,ipoinpc,iline, "*TIE%",ier)               "*TIE%",ier)
          cycle loop
        endif
        call getnewline(inpc,textpart,istat,n,key,iline,ipol,inl,&
             ipoinp,inp,ipoinpc)
        ...
        do
          ...
          if(igen)then
            if(n.lt.3)then
              ...
              call inputerror(inpc,ipoinpc,iline, "*TIME POINTS%",ier)                   "*TIME POINTS%",ier)
              exit
            else
        ...
        do i=2,n
          if(textpart(i)(1:10).eq.'CONSTANTS=') then
            read(textpart(i)(11:20),'(i10)',iostat=istat)&
                 nconstants
            if(istat.gt.0) then
              call inputerror(inpc,ipoinpc,iline, "*USER MATERIAL%",ier)                   "*USER MATERIAL%",ier)
              exit
            endif
