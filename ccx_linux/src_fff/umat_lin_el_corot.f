...
!
      ...
      !          write(*,*) '*ERROR in umat_lin_el_corot: the Young modulus'
      !          write(*,*) '       is too small'
      !          call exit(201)
      !       endif
      ! !
      !       if(elconloc(2).lt.1.d-30) then
      !          write(*,*) '*ERROR in umat_lin_el_corot: maximum pressure'
      ...
      e(3,1)=emec(5)
      e(3,2)=emec(6)
      !       if(iint.eq.1) write(*,*) 'emec'
      !       if(iint.eq.1) write(*,100) (emec(i),i=1,6)
      !
      n=3
      matz=1
      ier=0
      ...
      do i=1,3
         wc(i)=2.d0*we(i)+1.d0
      !          if(iint.eq.1)
      !      &       write(*,*) 'eigenvalues',i,wc(i),z(1,i),z(2,i),z(3,i)
      enddo
      !
      ...
      if((dabs(we(3)-we(2)).lt.1.d-10).and.&
         ...
         do i=4,6
            u(i)=0.d0
         enddo
         !          if(iint.eq.1) write(*,*) '1 diff u'
         !          if(iint.eq.1) write(*,100) (u(i),i=1,6)
         !
         if(icmd.ne.3) then
            ...
            enddo
         !             if(iint.eq.1) write(*,*) '1 diff dude'
         !             if(iint.eq.1) write(*,100) (dude(i),i=1,21)
         endif
      !
      ...
      elseif(dabs(we(3)-we(2)).lt.1.d-10) then
         ...
         enddo
         !          if(iint.eq.1) write(*,*) '2 diff u (w2=w3)'
         !          if(iint.eq.1) write(*,100) (u(i),i=1,6)
         !
         if(icmd.ne.3) then
            !
            ...
            do j=1,21
               ...
               dude(j)=2.d0*(dfla(1)*xmm1(j)&
                       ...
                       /(2.d0*(we(1)-we(2))))
            enddo
         !             if(iint.eq.1) write(*,*) '2 diff dude (w2=w3)'
         !             if(iint.eq.1) write(*,100) (dude(i),i=1,21)
         endif
      !
      ...
      elseif(dabs(we(2)-we(1)).lt.1.d-10) then
         ...
         enddo
         !          if(iint.eq.1) write(*,*) '2 diff u (w1=w2)'
         !          if(iint.eq.1) write(*,100) (u(i),i=1,6)
         !
         if(icmd.ne.3) then
            !
            ...
            do j=1,21
               ...
               dude(j)=2.d0*(dfla(3)*xmm3(j)&
                       ...
                       /(2.d0*(we(2)-we(3))))
            enddo
         !             if(iint.eq.1) write(*,*) '2 diff dude (w1=w2)'
         !             if(iint.eq.1) write(*,100) (dude(i),i=1,21)
         endif
      !
      ...
      !
         ...
         if(icmd.ne.3) then
            ...
            enddo
         !             if(iint.eq.1) write(*,*) '3 diff dude'
         !             if(iint.eq.1) write(*,100) (dude(i),i=1,21)
         endif
      endif
      !
      ...
      if(nconstants.eq.2) then
         ...
         if(icmd.ne.3) then
            !          if(iint.eq.1) then
            ! c     write(*,100) (stre(i),i=1,6)
            !             write(*,*) 'dude'
            !             write(*,100) (dude(i),i=1,6)
            !             write(*,100) (dude(i),i=7,10)
            !             write(*,100) (dude(i),i=11,15)
            !             write(*,100) (dude(i),i=16,21)
            !          endif
            !
         ...
         !             stiff(1)=stiff(1)+al*(dude(1)+dude(2)+dude(4))
         !             stiff(2)=stiff(2)+al*(dude(2)+dude(3)+dude(5))
         !             stiff(3)=stiff(3)+al*(dude(4)+dude(5)+dude(6))
         !             stiff(4)=stiff(4)+al*(dude(2)+dude(3)+dude(5))
         !             stiff(5)=stiff(5)+al*(dude(4)+dude(5)+dude(6))
         !             stiff(6)=stiff(6)+al*(dude(4)+dude(5)+dude(6))
         !          if(iint.eq.1) then
         ! c            write(*,100) (stre(i),i=1,6)
         !             write(*,*) 'stiff'
         !             write(*,100) (stiff(i),i=1,6)
         !             write(*,100) (stiff(i),i=7,10)
         !             write(*,100) (stiff(i),i=11,15)
         !             write(*,100) (stiff(i),i=16,21)
         !          endif
         endif
      endif
