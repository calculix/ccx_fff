...
!
      ...
      do i=2,n
         if(textpart(i)(1:4).eq.'USER') then
            ...
            call inputwarning(inpc,ipoinpc,iline, "*MATERIAL%")      "*MATERIAL%")
         endif
      enddo
      !
      ...
      do
         ...
         do i=1,3
            read(textpart(i)(1:20),'(f20.0)',iostat=istat)&
                 elcon(8+i,1,imat)
            if(istat.gt.0) then
               call inputerror(inpc,ipoinpc,iline, "*GAP HEAT GENERATION%",ier)                    "*GAP HEAT GENERATION%",ier)
               return
            endif
         enddo
         if(elcon(9,1,imat).le.0.d0) then
            write(*,*) '*ERROR reading *GAP HEAT GENERATION: fraction of'             '*ERROR reading *GAP HEAT GENERATION: fraction of'
         ...
         if((elcon(10,1,imat).lt.0.d0).or.&
            ...
            write(*,*) '*ERROR reading *GAP HEAT GENERATION: weighting factor'               '*ERROR reading *GAP HEAT GENERATION: weighting factor'
      ...
      if((.not.user).and.(nline.eq.0)) then
         write(*,*) '*ERROR reading *GAP HEAT GENERATION: no data given'              '*ERROR reading *GAP HEAT GENERATION: no data given'
         ier=1
         return
      endif
      !
