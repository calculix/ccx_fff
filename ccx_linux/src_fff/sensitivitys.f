      ...
      do i=2,n
         if(textpart(i)(1:4).eq.'READ') then
            if(iwrite) then
               ...
               write(*,*) '       exclusive'
               call inputerror(inpc,ipoinpc,iline, "*SENSITIVITY%",ier)                    "*SENSITIVITY%",ier)
               return
            endif
         ...
         elseif(textpart(i)(1:5).eq.'WRITE') then
            ...
            write(*,*) '*WARNING reading *SENSITIVITY: parameter not              '*WARNING reading *SENSITIVITY: parameter not&
      recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*SENSITIVITY%")      "*SENSITIVITY%")
         endif
      enddo
      !
      nmethod=12
