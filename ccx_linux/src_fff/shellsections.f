...
!
      ...
      if((istep.gt.0).and.(irstrt(1).ge.0)) then
         write(*,*) '*ERROR reading *SHELL SECTION: *SHELL SECTION should'              '*ERROR reading *SHELL SECTION: *SHELL SECTION should'
      ...
      do i=2,n
         ...
         elseif(textpart(i)(1:7).eq.'OFFSET=') then
            ...
            if(istat.gt.0) then
               call inputerror(inpc,ipoinpc,iline, "*SHELL SECTION%",ier)                    "*SHELL SECTION%",ier)
               return
            endif
         elseif(textpart(i)(1:9).eq.'COMPOSITE') then
            composite=.true.
         else
            write(*,*) '*WARNING reading *SHELL SECTION: parameter not recognized:'         '*WARNING reading *SHELL SECTION: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*SHELL SECTION%")      "*SHELL SECTION%")
         endif
      enddo
      !
      ...
      if(.not.composite) then
         ...
         enddo
         if(i.gt.nmat) then
            write(*,*) '*ERROR reading *SHELL SECTION: nonexistent material'              '*ERROR reading *SHELL SECTION: nonexistent material'
            call inputerror(inpc,ipoinpc,iline, "*SHELL SECTION%",ier)                 "*SHELL SECTION%",ier)
            return
         endif
         imaterial=i
      ...
      !          write(*,*) '*INFO reading *SHELL SECTION: an orientation'
      ...
      !          iorientation=0
         ...
         enddo
         if(i.gt.norien) then
            write(*,*) '*ERROR reading *SHELL SECTION: nonexistent orientation'               '*ERROR reading *SHELL SECTION: nonexistent orientation'
            call inputerror(inpc,ipoinpc,iline, "*SHELL SECTION%",ier)                 "*SHELL SECTION%",ier)
            return
         endif
         iorientation=i
      ...
      if(.not.composite) then
         if(.not.nodalthickness) then
            ...
            if(istat.gt.0) then
               write(*,*) '*ERROR reading *SHELL SECTION: shell thickness is lacking'          '*ERROR reading *SHELL SECTION: shell thickness is lacking'
               call inputerror(inpc,ipoinpc,iline, "*SHELL SECTION%",ier)                    "*SHELL SECTION%",ier)
               return
            endif
            if(thickness.le.0.d0) then
               write(*,*) '*ERROR reading *SHELL SECTION: shell thickness is zero'          '*ERROR reading *SHELL SECTION: shell thickness is zero'
               write(*,*) '       or negative'
               call inputerror(inpc,ipoinpc,iline, "*SHELL SECTION%",ier)                    "*SHELL SECTION%",ier)
            endif
            ...
            do j=istartset(iset),iendset(iset)
               if(ialset(j).gt.0) then
                  if(lakon(ialset(j))(1:1).ne.'S') then
                     write(*,*) '*ERROR reading *SHELL SECTION: *SHELL SECTION can'                   '*ERROR reading *SHELL SECTION: *SHELL SECTION can'
                     write(*,*) '       only be used for shell elements.'                       '       only be used for shell elements.'
                     write(*,*) '       Element ',ialset(j), ' is not a shell element.'                       ' is not a shell element.'
                     ier=1
                     return
                  endif
                  ...
                  do
                     ...
                     if(lakon(k)(1:1).ne.'S') then
                        write(*,*) '*ERROR reading *SHELL SECTION: *SHELL SECTION can'                  '*ERROR reading *SHELL SECTION: *SHELL SECTION can'
                        write(*,*) '       only be used for shell elements.'                          '       only be used for shell elements.'
                        write(*,*) '       Element ',k, ' is not a shell element.'                          ' is not a shell element.'
                        ier=1
                        return
                     endif
                     indexe=ipkon(k)
      ...
      !
         ...
         do
            ...
            if(istat.gt.0) then
               write(*,*) '*ERROR reading *SHELL SECTION: no material defined'                    '*ERROR reading *SHELL SECTION: no material defined'
               call inputerror(inpc,ipoinpc,iline, "*SHELL SECTION%",ier)                    "*SHELL SECTION%",ier)
               return
            endif
            !
            ...
            do j=istartset(iset),iendset(iset)
               if(ialset(j).gt.0) then
                  if((lakon(ialset(j))(1:3).ne.'S8R').and.&
                     ...
                     write(*,*) '*ERROR reading *SHELL SECTION: *SHELL SECTION'                 '*ERROR reading *SHELL SECTION: *SHELL SECTION'
                     write(*,*) '       with the option COMPOSITE can'                       '       with the option COMPOSITE can'
                     write(*,*) '       only be used for S8R or S6 shell elements.'                     '       only be used for S8R or S6 shell elements.'
                     write(*,*) '       Element ',ialset(j), ' is not a S8R nor a S6 shell element.'                       ' is not a S8R nor a S6 shell element.'
                     ier=1
                     return
                  endif
                  indexe=ipkon(ialset(j))
                  ...
                  do
                     ...
                     if((lakon(k)(1:3).ne.'S8R').and.&
                        ...
                        write(*,*) '    with the option COMPOSITE can'                             '    with the option COMPOSITE can'
                        write(*,*) '       only be used for S8R or S6 shell elements.'                     '       only be used for S8R or S6 shell elements.'
                        write(*,*) '       Element ',k, ' is not a S8R nor a S6 shell element.'                          ' is not a S8R nor a S6 shell element.'
                        ier=1
                        return
                     endif
                     indexe=ipkon(k)
