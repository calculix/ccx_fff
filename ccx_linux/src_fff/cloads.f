...
!
      ...
      do i=2,n
         ...
         elseif(textpart(i)(1:10).eq.'AMPLITUDE=') then
            ...
            if(j.gt.nam) then
               ...
               write(*,*) '  '
               call inputerror(inpc,ipoinpc,iline, "*CLOAD%",ier)                    "*CLOAD%",ier)
               return
            endif
            iamplitude=j
         elseif(textpart(i)(1:10).eq.'TIMEDELAY=') THEN
            if(idelay.ne.0) then
               write(*,*) '*ERROR reading *CLOAD: the parameter TIME DELAY'                  '*ERROR reading *CLOAD: the parameter TIME DELAY'
         ...
         elseif(textpart(i)(1:9).eq.'LOADCASE=') then
            ...
            if(nmethod.ne.5) then
               write(*,*) '*ERROR reading *CLOAD: the parameter LOAD CASE'                  '*ERROR reading *CLOAD: the parameter LOAD CASE'
         ...
         elseif(textpart(i)(1:7).eq.'OMEGA0=') then
            ...
            write(*,*) '*WARNING reading *CLOAD: parameter not recognized:'              '*WARNING reading *CLOAD: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*CLOAD%")      "*CLOAD%")
         endif
      enddo
      !
      ...
      do
         ...
         if((iforcdir.lt.1).or.(iforcdir.gt.6)) then
            write(*,*) '*ERROR reading *CLOAD: nonexistent degree of freedom'               '*ERROR reading *CLOAD: nonexistent degree of freedom'
            write(*,*) '       '
            call inputerror(inpc,ipoinpc,iline, "*CLOAD%",ier)                 "*CLOAD%",ier)
            return
         endif
                  ...
                  if(submodel) then
                     if(ntrans.gt.0) then
                        if(inotr(1,k).gt.0) then
                           write(*,*) '*ERROR reading *CLOAD: in submodel'                             '*ERROR reading *CLOAD: in submodel'
