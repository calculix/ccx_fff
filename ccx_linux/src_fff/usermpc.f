      ...
      if(node.ne.0) then
            ...
            if((label(1:7).ne.'MEANROT').and.&
               ...
               if(id.gt.0) then
                  if(ikmpc(id).eq.idof) then
                     ...
                     write(*,*) '         on the dependent side of another'                         '         on the dependent side of another'
                     write(*,*) '         MPC. ',label
         ...
         if((labmpc(nmpc)(1:7).eq.'MEANROT').or.&
            ...
            if(3*nkn.ne.nnodes-1) then
               write(*,*) '*ERROR in usermpc: MPC has wrong number of terms'                    '*ERROR in usermpc: MPC has wrong number of terms'
               call exit(201)
            endif
            !
            ...
            if(dd.lt.1.d-10) then
               write(*,*) '*ERROR in usermpc: rotation vector has zero length'                 '*ERROR in usermpc: rotation vector has zero length'
               call exit(201)
            endif
            do i=1,3
               aa(i)=aa(i)/dd
