...
!
      ...
      do ii=2,n
        if(textpart(ii)(1:10).eq.'FREQUENCY=') then
           ...
           if(istat.gt.0) then
              call inputerror(inpc,ipoinpc,iline, "*OUTPUT %",ier)                   "*OUTPUT %",ier)
              return
           endif
           if(joutl.eq.0) then
              do
        ...
        elseif(textpart(ii)(1:11).eq.'FREQUENCYF=') then
           ...
           if(joutl.gt.0) then
            ...
            write(*,*) '*WARNING reading *NODE FILE or *EL FILE:'                   '*WARNING reading *NODE FILE or *EL FILE:'
            write(*,*) '         parameter not recognized:'
            write(*,*) '         ',&
                       textpart(ii)(1:index(textpart(ii),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*OUTPUT %")      "*OUTPUT %")
        endif
      enddo
      !
