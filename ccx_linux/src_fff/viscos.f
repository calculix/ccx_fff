...
!
      ...
      do i=2,n
         ...
         elseif(textpart(i)(1:6).eq.'CETOL=') then
            ...
            if(istat.gt.0) then
               call inputerror(inpc,ipoinpc,iline, "*VISCO%",ier)                    "*VISCO%",ier)
               return
            endif
         else
            write(*,*) '*WARNING reading *VISCO: parameter not recognized:'              '*WARNING reading *VISCO: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*VISCO%")      "*VISCO%")
         endif
      enddo
      !
