...
!
      ...
      if(istep.lt.1) then
         ...
         write(*,*) '       *INITIAL STRAIN INCREASE should only be used'           '       *INITIAL STRAIN INCREASE should only be used'
         write(*,*) '  within a STEP'
         ier=1
         return
      endif
      ...
      do i=2,n
         ...
         call inputwarning(inpc,ipoinpc,iline, "*INITIAL STRAIN INCREASE%")      "*INITIAL STRAIN INCREASE%")
      enddo
      !
      ...
      do
         ...
         do j=1,6
            read(textpart(j+2)(1:20),'(f20.0)',iostat=istat)&
                 beta(j)
            if(istat.gt.0) then
               call inputerror(inpc,ipoinpc,iline, "*INITIAL STRAIN INCREASE%",ier)                    "*INITIAL STRAIN INCREASE%",ier)
               return
            endif
         enddo
         ...
         if(l.gt.ne) then
            write(*,*) '*WARNING reading *INITIAL STRAIN INCREASE: element ',l                 '*WARNING reading *INITIAL STRAIN INCREASE: element ',l
