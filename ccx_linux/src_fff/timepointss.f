...
!
      ...
      do i=2,n
         if(textpart(i)(1:5).eq.'NAME=') then
            ...
            if(textpart(i)(86:86).ne.' ') then
               write(*,*) '*ERROR reading *TIME POINTS: amplitude name too long'                 '*ERROR reading *TIME POINTS: amplitude name too long'
         ...
         elseif(textpart(i)(1:8).eq.'GENERATE') then
            ...
            write(*,*) '*WARNING reading *TIME POINTS: parameter not recognized:'              '*WARNING reading *TIME POINTS: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*TIME POINTS%")      "*TIME POINTS%")
         endif
      enddo
      !
      if(amname(nam).eq.' ') then                                       ') then
            ...
            do i=1,8
               if(textpart(i)(1:1).ne.' ') then  
                  ...
                  if(namtot.gt.namtot_) then
                     write(*,*) '*ERROR reading *TIME POINTS: increase namtot_'                      '*ERROR reading *TIME POINTS: increase namtot_'
                     ier=1
                     return
                  endif
      ...
      if(namta(1,nam).gt.namta(2,nam)) then
         ...
         write(*,*) '*WARNING reading *TIME POINTS: *TIME POINTS definition ',          '*WARNING reading *TIME POINTS: *TIME POINTS definition ',&
              amname(nam)(1:ipos-1)
