        ...
        if(rig(node).ne.0) then
          ...
          if((idir.gt.3).and.((nmethod.eq.4).and.(iperturb(1).gt.1)))&
            ...
            elseif(lakon(ielem)(7:7).eq.'B') then
              ...
              write(*,*) '*ERROR in gen3dboun: a rotational DOF was applied'                   '*ERROR in gen3dboun: a rotational DOF was applied'
              write(*,*) '*      to node',node,' without rotational DOFs'                   '*      to node',node,' without rotational DOFs'
              call exit(201)
            endif
            !
            ...
            if(dd.lt.1.d-20) then
              ...
              if(idim.eq.2) then
                ...
                if(ier.ne.0) then
                  write(*,*) '*ERROR in knotmpc while calculating the'                       '*ERROR in knotmpc while calculating the'
              ...
              if(dabs(1.d0-dmax).lt.1.d-3) then
                ...
                if(isol.eq.1) then
                  ...
                  if(nmpc.gt.nmpc_) then
                    write(*,*) '*ERROR in gen3dboun: increase nmpc_'                         '*ERROR in gen3dboun: increase nmpc_'
                    call exit(201)
                  endif
                  !
                  ipompc(nmpc)=mpcfree
          ...
          if((idir.gt.3).and.((nmethod.ne.4).or.(iperturb(1).le.1)))&
            ...
            if(lakon(ielem)(7:7).eq.'L') then
              ...
              !      do k=1,3
              !      idofr=8*(node-1)+k+3
              !      call nident(ikboun,idofr,nboun,idr(k))
              !      if(idr(k).gt.0) then
              !      if(ikboun(idr(k)).eq.idofr) then
              !      ispc=ilboun(idr(k))
              !      if(ne2boun(1,ispc).ne.0) then
              !      idir1=k
              !      ispcref=ispc
              !      ispc1=ne2boun(1,ispcref)
              !      ispc2=ne2boun(2,ispcref)
              !      endif
              !      cycle
              !      endif
              !      endif
              ...
              if(ispc1.eq.0) then
                ...
                !
                label='MEANROTBS           '
                write(27,*) 'a MEAN ROTATION MPC was generated in node ',                     'a MEAN ROTATION MPC was generated in node ',&
                     node
                ...
                if(nodeact.ne.-1) then
                  ...
                  !     value in ne2boun
                  !
                  !      ne2boun(1,ispcref)=nboun
                  ne2boun(1,node)=nboun
                endif
              !      elseif(idirref.eq.idir1) then
                ...
                do idir=1,3
                  ...
                  if((id.le.0).or.(ikmpc(id).ne.idof)) then
                    ...
                    if(mpcfree.eq.0) then
                      write(*,*) '*ERROR in gen3dboun: increase memmpc_'                           '*ERROR in gen3dboun: increase memmpc_'
                      call exit(201)
                    endif
          ...
          elseif(lakon(ielem)(7:7).eq.'B') then
            ...
            if(((id.le.0).or.(ikmpc(id).ne.idof)).and.&
              ...
              nmpc=nmpc+1
              if(nmpc.gt.nmpc_) then
                write(*,*) '*ERROR in gen3dmpc: increase nmpc_'                     '*ERROR in gen3dmpc: increase nmpc_'
                call exit(201)
              endif
              labmpc(nmpc)='                    '
              ipompc(nmpc)=mpcfree
              ...
              mpcfree=nodempc(3,mpcfree)
              if(mpcfree.eq.0) then
                write(*,*) '*ERROR in gen3dmpc: increase memmpc_'                     '*ERROR in gen3dmpc: increase memmpc_'
                call exit(201)
              endif
              nodempc(1,mpcfree)=node
              nodempc(2,mpcfree)=idir
