...
!
      ...
      if((istep.gt.0).and.(irstrt(1).ge.0)) then
         write(*,*) '*ERROR reading *USER MATERIAL: *USER MATERIAL should be'         '*ERROR reading *USER MATERIAL: *USER MATERIAL should be'
      ...
      do i=2,n
         ...
         elseif(textpart(i)(1:12).eq.'TYPE=THERMAL') then
            ...
            write(*,*) '*WARNING reading *USER MATERIAL: parameter not recognized:'            '*WARNING reading *USER MATERIAL: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*USER MATERIAL%")      "*USER MATERIAL%")
         endif
      enddo
      !
      if(imech.eq.1) then
            ...
            do j=1,(nconstants)/8+1
               if(j.eq.1) then
                  ...
                  if(ntmat.gt.ntmat_) then
                     write(*,*) '*ERROR reading *USER MATERIAL: increase ntmat_'                        '*ERROR reading *USER MATERIAL: increase ntmat_'
                     ier=1
                     return
                  endif
               else
                  ...
                  if((istat.lt.0).or.(key.eq.1)) then
                     write(*,*) '*ERROR reading *USER MATERIAL: anisotropic definition'                 '*ERROR reading *USER MATERIAL: anisotropic definition'
