...
!
      ...
      if((istep.eq.1).and.(nstam.eq.1)) then
         ...
         nam=nam+1
         if(nam.gt.nam_) then
            write(*,*) '*ERROR reading *STEP: increase nam_'                 '*ERROR reading *STEP: increase nam_'
            ier=1
            return
         endif
         namta(3,nam)=nam
         ...
         if(namtot.gt.namtot_) then
            write(*,*) '*ERROR reading *STEP: increase namtot_'                 '*ERROR reading *STEP: increase namtot_'
            ier=1
            return
         endif
         amta(1,namtot)=0.d0
      ...
      do i=2,n
         ...
         elseif((textpart(i)(1:6).eq.'NLGEOM').and.&
            ...
            elseif(iperturb(1).eq.1) then
               write(*,*) '*ERROR reading *STEP: PERTURBATION and NLGEOM'                  '*ERROR reading *STEP: PERTURBATION and NLGEOM'
               write(*,*) '       are mutually exclusive; '
               call inputerror(inpc,ipoinpc,iline, "*STEP%",ier)                    "*STEP%",ier)
               return
            endif
         !
         ...
         elseif(textpart(i)(1:15).eq.'SHOCKSMOOTHING=') then
            ...
            if(istat.gt.0) call inputerror(inpc,ipoinpc,iline, "*STEP%",ier)                    "*STEP%",ier)
         else
            write(*,*) '*WARNING reading *STEP: parameter not recognized:'                '*WARNING reading *STEP: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*STEP%")      "*STEP%")
         endif
      enddo
      !
