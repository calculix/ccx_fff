...
!
      ...
      do i=2,n
         if(textpart(i)(1:5).eq.'TYPE=') then
            ...
            call inputwarning(inpc,ipoinpc,iline, "*MAGNETIC PERMEABILITY%")      "*MAGNETIC PERMEABILITY%")
         endif
      enddo
      !
      nelcon(1,nmat)=ityp
      ...
      if(ityp.eq.2) then
         do
            ...
            read(textpart(1)(1:20),'(f20.0)',iostat=istat)&
                 elcon(1,ntmat,nmat)
            if(istat.gt.0) then
               call inputerror(inpc,ipoinpc,iline, "*MAGNETIC PERMEABILITY%",ier)                    "*MAGNETIC PERMEABILITY%",ier)
               return
            endif
            !
