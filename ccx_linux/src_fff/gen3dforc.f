      ...
      do i=1,nforcold
        ...
        if(node.gt.iponoelmax) then
          if(ndirforc(i).gt.3) then
            write(*,*) '*WARNING: in gen3dforc: node ',i, ' does not'                 ' does not'
            ...
            if(dabs(1.d0-dmax).lt.1.d-3) then
              ...
              if(isol.eq.1) then
                ...
                if(nmpc.gt.nmpc_) then
                  write(*,*) '*ERROR in gen3dnor: increase nmpc_'                       '*ERROR in gen3dnor: increase nmpc_'
                  call exit(201)
                endif
                !
                ipompc(nmpc)=mpcfree
        ...
        if((idir.gt.3).and.((nmethod.ne.4).or.(iperturb(1).le.1)))then
          ...
          if(lakon(ielem)(7:7).eq.'L') then
            ...
            if(dot.gt.999d0) then
              ...
              write(*,*) '         is nearly'
              write(*,*) '         along the drilling'                   '         along the drilling'
        ...
        if(lakon(ielem)(7:7).eq.'L') then
          ...
          do idirl=idirstart,idirend
            ...
            if((id.le.0).or.(ikmpc(id).ne.idof)) then
              nmpc=nmpc+1
              if(nmpc.gt.nmpc_) then
                write(*,*) '*ERROR in gen3dforc: increase nmpc_'                     '*ERROR in gen3dforc: increase nmpc_'
                call exit(201)
              endif
              labmpc(nmpc)='                    '
              ipompc(nmpc)=mpcfree
              ...
              if(mpcfree.eq.0) then
                write(*,*) '*ERROR in gen3dforc: increase memmpc_'                     '*ERROR in gen3dforc: increase memmpc_'
                call exit(201)
              endif
              !
