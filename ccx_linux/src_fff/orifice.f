      ...
      elseif(iflag.eq.3) then
         ...
         if(iflag.eq.3)then
          ...
          xmach=dsqrt(((p1/p2)**((kappa-1.d0)/kappa)-1.d0)*2.d0/&
                (kappa-1.d0))
          write(1,*) ''
          write(1,55) ' from node ',node1, ' to node ', node2,' :   air massflow rate = '         ' to node ', node2,' :   air massflow rate = '&
      ,inv*xflow,' ', ', oil massflow rate = ',xflow_oil,' '         ', oil massflow rate = ',xflow_oil,' '
          !
          if(inv.eq.1) then
             ...
             if(lakon(nelem)(2:5).eq.'ORC1') then
                ...
                write(1,59)'             CD = ',cd,' , C1u = ',u, '  , C2u = ', prop(index+7), ' '        '  , C2u = ', prop(index+7), ' '
             endif
             !     special for bleed tappings
             if(lakon(nelem)(2:5).eq.'ORBT') then
                write(1,63) '             P2/P1 = ',p2/p1, ' , ps1pt1 = ', ps1pt1, ' , DAB = ',(1-p2/p1)/(1-ps1pt1),      ' , ps1pt1 = ', ps1pt1, ' , DAB = ',(1-p2/p1)/(1-ps1pt1), ' , curve N° = ', curve,' , cd = ',cd      ' , curve N° = ', curve,' , cd = ',cd
             !     special for preswirlnozzles
             elseif(lakon(nelem)(2:5).eq.'ORPN') then
                write(1,62) '             cd = ', cd, '  , C2u = ',c2u_new,' '      '  , C2u = ',c2u_new,' '
