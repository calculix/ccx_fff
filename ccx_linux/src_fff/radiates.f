...
!
      ...
      !
      if(istep.lt.1) then
        write(*,*) '*ERROR reading *RADIATE: *RADIATE should only be used'             '*ERROR reading *RADIATE: *RADIATE should only be used'
        write(*,*) '  within a STEP'
        ier=1
        return
      endif
      ...
      if(physcon(2).le.0.d0) then
        write(*,*) '*ERROR reading *RADIATE: *RADIATE card was selected'             '*ERROR reading *RADIATE: *RADIATE card was selected'
      ...
      do i=2,n
        ...
        elseif(textpart(i)(1:10).eq.'AMPLITUDE=') then
          ...
          enddo
          if(j.eq.0) then
            write(*,*) '*ERROR reading *RADIATE: nonexistent amplitude'                 '*ERROR reading *RADIATE: nonexistent amplitude'
            write(*,*) '  '
            call inputerror(inpc,ipoinpc,iline, "*RADIATE%",ier)                 "*RADIATE%",ier)
            return
          endif
          iamptemp=j
        elseif(textpart(i)(1:10).eq.'TIMEDELAY=') THEN
          if(idelay1.ne.0) then
            write(*,*) '*ERROR reading *RADIATE: the parameter TIME DELAY'                 '*ERROR reading *RADIATE: the parameter TIME DELAY'
        ...
        elseif(textpart(i)(1:19).eq.'RADIATIONTIMEDELAY=') THEN
          if(idelay2.ne.0) then
            write(*,*) '*ERROR reading *RADIATE: the parameter RADIATION'                 '*ERROR reading *RADIATE: the parameter RADIATION'
          ...
          if(iampradi.eq.0) then
            write(*,*) '*ERROR reading *RADIATE: radiation time delay'                 '*ERROR reading *RADIATE: radiation time delay'
        ...
        elseif(textpart(i)(1:7).eq.'CAVITY=') THEN
          ...
          write(*,*) '*WARNING reading *RADIATE: parameter not recognized:'               '*WARNING reading *RADIATE: parameter not recognized:'
          write(*,*) '         ',&
               textpart(i)(1:index(textpart(i),' ')-1)
          call inputwarning(inpc,ipoinpc,iline, "*RADIATE%")               "*RADIATE%")
        endif
      enddo
      !
      do
