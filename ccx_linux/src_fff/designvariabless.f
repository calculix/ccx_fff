...
!
      ...
      do i=2,n
         if(textpart(i)(1:5).eq.'TYPE=') then
            read(textpart(i)(6:85),'(a80)',iostat=istat)&
                 tieset(1,ntie)(1:80)
            if(istat.gt.0) then
               call inputerror(inpc,ipoinpc,iline, "*DESIGNVARIABLE%",ier)                    "*DESIGNVARIABLE%",ier)
               return
            endif
            itype=1
         endif
      ...
      !
      if(itype.eq.0) then
         write(*,*) '*ERROR reading *DESIGN VARIABLES: type is lacking'      '*ERROR reading *DESIGN VARIABLES: type is lacking'
         call inputerror(inpc,ipoinpc,iline, "*DESIGNVARIABLE%",ier)              "*DESIGNVARIABLE%",ier)
         return
      endif
      !
      ...
      if(tieset(1,ntie)(1:10).eq.'COORDINATE') then
         ...
         if((istat.lt.0).or.(key.eq.1)) then
            write(*,*) '*ERROR reading *DESIGN VARIABLES: definition'      '*ERROR reading *DESIGN VARIABLES: definition'
         ...
         if(i.gt.nset) then
            ...
            write(*,*) 'node set ',tieset(2,ntie)(1:ipos-1), 'does not exist. Card image:'                 'does not exist. Card image:'
            call inputerror(inpc,ipoinpc,iline, "*DESIGN VARIABLES%",ier)                 "*DESIGN VARIABLES%",ier)
            return
         endif
      endif
      !
