      ...
      do idir=jstart,jend
         ...
         if((id.le.0).or.(ikmpc(id).ne.idof)) then
            ...
            if(nmpc.gt.nmpc_) then
               write(*,*) '*ERROR in gen3dtruss: increase nmpc_'                    '*ERROR in gen3dtruss: increase nmpc_'
               call exit(201)
            endif
            labmpc(nmpc)='                    '
            ipompc(nmpc)=mpcfree
            ...
            if(mpcfree.eq.0) then
               write(*,*) '*ERROR in gen3dtruss: increase memmpc_'                    '*ERROR in gen3dtruss: increase memmpc_'
               call exit(201)
            endif
            do k=2,4
