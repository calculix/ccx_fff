...
!
      ...
      if((istep.gt.0).and.(irstrt(1).ge.0)) then
         write(*,*) '*ERROR reading *NSET/ELSET: *NSET/*ELSET should be placed'           '*ERROR reading *NSET/ELSET: *NSET/*ELSET should be placed'
         ...
         do i=2,n
            if(textpart(i)(1:5).eq.'NSET=') then
               ...
               if(textpart(i)(86:86).ne.' ') then
                  write(*,*) '*ERROR reading *NSET/ELSET: set name too long'                    '*ERROR reading *NSET/ELSET: set name too long'
            ...
            elseif(textpart(i)(1:8).eq.'GENERATE') then
               ...
               write(*,*) '*WARNING reading *NSET/ELSET: parameter not recognized:'               '*WARNING reading *NSET/ELSET: parameter not recognized:'
               write(*,*) '         ',&
                    textpart(i)(1:index(textpart(i),' ')-1)
               call inputwarning(inpc,ipoinpc,iline, "*NSET or *ELSET%")      "*NSET or *ELSET%")
            endif
         enddo
      else
         do i=2,n
         ...
         if(set(iset).eq.noelset) then
            ...
            if(iendset(iset).eq.nalset) then
               ...
               if(nalset+nn.gt.nalset_) then
                  write(*,*) '*ERROR reading *NSET/ELSET: increase nalset_'                     '*ERROR reading *NSET/ELSET: increase nalset_'
                  ier=1
                  return
               endif
               do k=1,nn
         ...
         if(igen) then
            ...
            if(kode.eq.0) then
               if(ialset(nalset+1).gt.nk) then
                  write(*,*) '*ERROR reading *NSET/ELSET: starting value in'                     '*ERROR reading *NSET/ELSET: starting value in'
                  write(*,*) '       set ',&
                    set(iset)(1:index(set(iset),' ')-2),' > nk'
                  ier=1
                  return
               elseif(ialset(nalset+2).gt.nk) then
                  write(*,*) '*WARNING reading *NSET/ELSET: end value in'                     '*WARNING reading *NSET/ELSET: end value in'
                  write(*,*) '         set ',&
                    set(iset)(1:index(set(iset),' ')-2),' > nk;'
            ...
            do i=1,n
               ...
               if(istat.gt.0) then
                  ...
                  if(noelset.ne.set(j)) then
                     noelset(ipos:ipos)=' '
                     if(kode.eq.0) then
                        write(*,*) '*ERROR reading *NSET/ELSET: node set ',                            '*ERROR reading *NSET/ELSET: node set ',&
                          noelset
                     else
                        write(*,*) '*ERROR reading *NSET/ELSET: element set ',                          '*ERROR reading *NSET/ELSET: element set ',&
                          noelset
                     endif
                  ...
                  if(kode.eq.0) then
                     if(ialset(nalset+1).gt.nk) then
                        write(*,*) '*WARNING reading *NSET/ELSET: value ',                           '*WARNING reading *NSET/ELSET: value ',&
                             ialset(nalset+1)
                        write(*,*) '         in set ',&
                          set(iset)(1:index(set(iset),' ')-2),' > nk'
                     else
                     ...
                     if(ialset(nalset+1).gt.ne) then
                        ...
                        write(*,*) '         global elsets in combination'                             '         global elsets in combination'
