...
!
      ...
      if(network.gt.2) then
         ...
         !
         if(pressmin.lt.0.d0) then
            write(*,*) '*ERROR in initialnet: minimum initial pressure'                 '*ERROR in initialnet: minimum initial pressure'
      ...
      if(network.gt.2)then
         do i=1,nflow
            ...
            if((lakon(nelem)(2:3).eq.'LI').or.&
               ...
               if(.not.gravity) then
                  write(*,*) '*WARNING in initialnet: no gravity vector'                       '*WARNING in initialnet: no gravity vector'
                  write(*,*) '       was defined for liquid element',nelem                       '       was defined for liquid element',nelem
                  z1=0.d0
                  z2=0.d0
               !                   call exit(201)
               endif
            ...
            if(((lakon(nelem)(2:3).ne.'LI').and.&
               ...
               !
                  ...
                  write(*,*) '          no pressure gradient'
                  write(*,*) '          in element', nelem
                  write(*,*) '          Inlet pressure initial condition '                     '          Inlet pressure initial condition '
                  write(*,*) '          is changed '
                  write(*,*) '          node1',node1, ' given initial pressure',v(2,node1)                       ' given initial pressure',v(2,node1)
                  v(2,node1)=1.1d0*v(2,node1)
                  write(*,*) '          node1',node1, ' new initial pressure',v(2,node1)                       ' new initial pressure',v(2,node1)
                  write(*,*) '          node2',node2,' pressure',&
                       v(2,node2)
               !
               ...
               !
                  ...
                  write(*,*) '          no pressure gradient'
                  write(*,*) '          in element', nelem
                  write(*,*) '          Outlet pressure initial condition '                       '          Outlet pressure initial condition '
                  write(*,*) '          is changed '
                  write(*,*) '          node1',node1,' pressure'&
                       ,v(2,node1)
                  write(*,*) '          node2',node2, 'given intial pressure',                       'given intial pressure',&
                       v(2,node2)
                  v(2,node2)=0.9d0*v(2,node2)
                  write(*,*) '          node2',node2, ' new initial pressure',v(2,node2)                       ' new initial pressure',v(2,node2)
               !
               ...
               !
                  ...
                  write(*,*) '          node1',node1, ' new intial pressure',v(2,node1)                       ' new intial pressure',v(2,node1)
                  write(*,*) '          node2',node2, ' given initial pressure',v(2,node2)                       ' given initial pressure',v(2,node2)
                  v(2,node2)=0.95d0*v(2,node2)
                  write(*,*) '          node2',node2, ' new intial pressure',v(2,node2)                       ' new intial pressure',v(2,node2)
               endif
            endif
            !
            ...
            if((lakon(nelem)(2:4).ne.'LIP').and.&
               ...
               if (v(1,nodem)*(v(2,node1)-v(2,node2)).lt.0) then
                  ...
                  write(*,*) '*WARNING: in subroutine initialnet.f'
                  write(*,*) '          in element', nelem
                  write(*,*) '          initial mass flow rate value does not'                       '          initial mass flow rate value does not'
                  ...
                  write(*,*) '          nodem',nodem,'mass flow',&
                       v(1,nodem)
                  write(*,*) '          the sign of the initial mass flow'                     '          the sign of the initial mass flow'
      ...
      if (gaspipe.and.(iin_abs.eq.0)) then
         ...
         do i=1,ntg
            if(ineighe(i).gt.2) then
               ...
               write(*,*) '          node is converted into a chamber.'                    '          node is converted into a chamber.'
