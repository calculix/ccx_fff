...
!
      ...
      !
      if(istep.lt.1) then
        write(*,*) '*ERROR reading *REFINE MESH: *REFINE MESH'             '*ERROR reading *REFINE MESH: *REFINE MESH'
      ...
      do ii=2,n
        ...
        elseif(textpart(ii)(1:6).eq.'ELSET=') THEN
          ...
          if(textpart(ii)(67:67).ne.' ') then
            ...
            call inputerror(inpc,ipoinpc,iline, "*REFINE MESH%",ier)                 "*REFINE MESH%",ier)
            return
          endif
          do i=61,81
            elset(i:i)=' '
          ...
          write(*,*) '*WARNING reading *REFINE MESH:'               '*WARNING reading *REFINE MESH:'
          write(*,*) '         parameter not recognized:'
          write(*,*) '         ',&
               textpart(ii)(1:index(textpart(ii),' ')-1)
          call inputwarning(inpc,ipoinpc,iline, "*REFINE MESH %")               "*REFINE MESH %")
        endif
      enddo
      !
      ...
      do
        ...
        do ii=1,n
          if((textpart(ii)(1:4).eq.'ERR ').or.&
            ...
            write(*,*) '*WARNING reading *REFINE MESH: label not applicable'                 '*WARNING reading *REFINE MESH: label not applicable'
            write(*,*) '         or unknown; '
            call inputwarning(inpc,ipoinpc,iline, "*REFINE MESH %")                 "*REFINE MESH %")
          endif
        enddo
