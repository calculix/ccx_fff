...
!
      ...
      do i=2,n
         ...
         elseif(textpart(i)(1:25).eq.'PRESSURE-OVERCLOSURE=HARD') then
            ...
            call inputwarning(inpc,ipoinpc,iline, "*SURFACE BEHAVIOR%")      "*SURFACE BEHAVIOR%")
            pressureoverclosure='L'
         else
            write(*,*) '*WARNING reading *SURFACE BEHAVIOR: parameter not recognized:'         '*WARNING reading *SURFACE BEHAVIOR: parameter not recognized:'
            write(*,*) '         ',&
      ...
      elseif((pressureoverclosure.eq.'L').or.&
         ...
         if((istat.lt.0).or.(key.eq.1)) then
            ...
            write(*,*) '         the following default will be used:',             '         the following default will be used:',&
                        50.d0*elcon(1,1,1)
            write(*,*) '         the user is advised to analyze the results'             '         the user is advised to analyze the results'
            write(*,*) '         carefully' 
            write(*,*)
            elcon(2,1,imat)=50.d0*elcon(1,1,1)
            write(*,*) '*INFO reading *SURFACE BEHAVIOR: the tension'
            write(*,*) '      value at large clearances'
            write(*,*) '      (only relevant for node-to-surface contact)'            '      (only relevant for node-to-surface contact)'
            write(*,*) '      must be strictly positive'
            write(*,*) '      the following default will be used:',             '      the following default will be used:',&
                        elcon(1,1,1)/70000.d0
            write(*,*) '      the user is advised to analyze the results'             '      the user is advised to analyze the results'
            write(*,*) '      carefully'             '      carefully'
            write(*,*)
         ...
         if(elcon(2,1,imat).le.0.d0) then
            ...
            write(*,*) '         the following default will be used:',             '         the following default will be used:',&
                        50.d0*elcon(1,1,1)
            write(*,*) '         the user is advised to analyze the results'             '         the user is advised to analyze the results'
            write(*,*) '         carefully and, if possible, to come up with'             '         carefully and, if possible, to come up with'
            write(*,*) '         a experimentally based K'             '         a experimentally based K'
            call inputwarning(inpc,ipoinpc,iline, "*SURFACE BEHAVIOR%")      "*SURFACE BEHAVIOR%")
            elcon(2,1,imat)=50.d0*elcon(1,1,1)
         endif
         !
      ...
      elseif(pressureoverclosure.eq.'T') then
         ...
         do
            ...
            if(npmat.gt.npmat_) then
               write(*,*) '*ERROR reading *SURFACE BEHAVIOR: increase npmat_'                    '*ERROR reading *SURFACE BEHAVIOR: increase npmat_'
               ier=1
               return
            endif
         ...
         !
         if(ntmat.eq.0) then
            write(*,*) '*ERROR reading *SURFACE BEHAVIOR: *SURFACE BEHAVIOR'                 '*ERROR reading *SURFACE BEHAVIOR: *SURFACE BEHAVIOR'
