...
!
      ...
      !
      if(nmat.eq.0) then
         write(*,*) '*ERROR reading *PLASTIC: *PLASTIC should be preceded'            '*ERROR reading *PLASTIC: *PLASTIC should be preceded'
      ...
      do i=2,n
         if(textpart(i)(1:10).eq.'HARDENING=') then
            ...
            write(*,*) '*WARNING reading *PLASTIC: parameter not recognized:'              '*WARNING reading *PLASTIC: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*PLASTIC%")      "*PLASTIC%")
         endif
      enddo
      !
      if(iso) then
      ...
      !
      if(ntmat.eq.0) then
         write(*,*) '*ERROR reading *PLASTIC: *PLASTIC card without data'             '*ERROR reading *PLASTIC: *PLASTIC card without data'
         ier=1
         return
      endif
      !
      ...
      if(nelcon(1,nmat).eq.-114) then
         if(matname(nmat)(71:80).ne.'          ') then
            write(*,*) '*ERROR reading *PLASTIC: the material name for an'                '*ERROR reading *PLASTIC: the material name for an'
         ...
         if(iso) then
            ...
            do i=1,nelcon(2,nmat)
               ...
               !                plconloc(2)=0.d0
               !                plconloc(3)=0.d0
               !                plconloc(81)=nplicon(1,nmat)+0.5d0
               !
            ...
            if(ndatamax.gt.2) then
               write(*,*) '*WARNING reading *PLASTIC: isotropic hardening'                  '*WARNING reading *PLASTIC: isotropic hardening'
            ...
            do i=1,nelcon(2,nmat)
               ...
               !                plconloc(2)=0.d0
               !                plconloc(3)=0.d0
               !                plconloc(82)=nplkcon(1,nmat)+0.5d0
               !
            ...
            enddo
            if(ndatamax.gt.2) then
               write(*,*) '*WARNING reading *PLASTIC: kinematic hardening'                   '*WARNING reading *PLASTIC: kinematic hardening'
               write(*,*) '         curve is possibly nonlinear for'
               write(*,*) '         the elastically anisotropic'
               write(*,*) '         material ',matname(nmat)(11:80)
            endif
      ...
      !
      !       if(nelcon(1,nmat).eq.-114) then
      !          write(*,*) 'anisotropic elasticity+viscoplasticity'
      !          do i=1,nelcon(2,nmat)
      !             write(*,*) (elcon(j,i,nmat),j=0,14)
      !          enddo
      !       endif
      !
      return
