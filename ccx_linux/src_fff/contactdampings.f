      ...
      do i=2,n
         if(textpart(i)(1:16).eq.'TANGENTFRACTION=') then
            read(textpart(i)(17:36),'(f20.0)',iostat=istat)&
                     elcon(8,1,imat)
            if(istat.gt.0) then
               call inputerror(inpc,ipoinpc,iline, "*CONTACT DAMPING%",ier)                    "*CONTACT DAMPING%",ier)
               return
            endif
         else
            write(*,*) '*WARNING reading *CONTACT DAMPING: parameter not recognized:'         '*WARNING reading *CONTACT DAMPING: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*CONTACT DAMPING%")      "*CONTACT DAMPING%")
         endif
      enddo
      !
