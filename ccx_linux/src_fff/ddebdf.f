      ...
      IF (IWORK(LIW).GE. 5) THEN
         IF (T .EQ. RWORK(21+NEQ)) THEN
            ...
            CALL XERMSG ('SLATEC', 'DDEBDF', 'AN APPARENT INFINITE LOOP HAS BEEN DETECTED.$$' //               'AN APPARENT INFINITE LOOP HAS BEEN DETECTED.$$' // 'YOU HAVE MADE REPEATED CALLS AT T = ' // XERN3 //               'YOU HAVE MADE REPEATED CALLS AT T = ' // XERN3 // ' AND THE INTEGRATION HAS NOT ADVANCED.  CHECK THE ' //               ' AND THE INTEGRATION HAS NOT ADVANCED.  CHECK THE ' // 'WAY YOU HAVE SET PARAMETERS FOR THE CALL TO THE ' //               'WAY YOU HAVE SET PARAMETERS FOR THE CALL TO THE ' // 'CODE, PARTICULARLY INFO(1).', 13, 2)               'CODE, PARTICULARLY INFO(1).', 13, 2)
            RETURN
         ENDIF
      ENDIF
      !
      ...
      IF (INFO(1) .NE. 0 .AND. INFO(1) .NE. 1) THEN
         ...
         CALL XERMSG ('SLATEC', 'DDEBDF', 'INFO(1) MUST BE SET TO 0 ' // 'FOR THE  START OF A NEW PROBLEM, AND MUST BE SET TO 1 ' //            'FOR THE  START OF A NEW PROBLEM, AND MUST BE SET TO 1 ' // 'FOLLOWING AN INTERRUPTED TASK.  YOU ARE ATTEMPTING TO ' //            'FOLLOWING AN INTERRUPTED TASK.  YOU ARE ATTEMPTING TO ' // 'CONTINUE THE INTEGRATION ILLEGALLY BY CALLING THE ' //            'CONTINUE THE INTEGRATION ILLEGALLY BY CALLING THE ' // 'CODE WITH  INFO(1) = ' // XERN1, 3, 1)            'CODE WITH  INFO(1) = ' // XERN1, 3, 1)
         IDID = -33
      ENDIF
      !
      IF (INFO(2) .NE. 0 .AND. INFO(2) .NE. 1) THEN
         WRITE (XERN1, '(I8)') INFO(2)
         CALL XERMSG ('SLATEC', 'DDEBDF', 'INFO(2) MUST BE 0 OR 1 ' // 'INDICATING SCALAR AND VECTOR ERROR TOLERANCES, ' //            'INDICATING SCALAR AND VECTOR ERROR TOLERANCES, ' // 'RESPECTIVELY.  YOU HAVE CALLED THE CODE WITH INFO(2) = ' //            'RESPECTIVELY.  YOU HAVE CALLED THE CODE WITH INFO(2) = ' //&
            XERN1, 4, 1)
         IDID = -33
      ENDIF
      !
      IF (INFO(3) .NE. 0 .AND. INFO(3) .NE. 1) THEN
         ...
         CALL XERMSG ('SLATEC', 'DDEBDF', 'INFO(3) MUST BE 0 OR 1 ' // 'INDICATING THE INTERVAL OR INTERMEDIATE-OUTPUT MODE OF ' //            'INDICATING THE INTERVAL OR INTERMEDIATE-OUTPUT MODE OF ' // 'INTEGRATION, RESPECTIVELY.  YOU HAVE CALLED THE CODE ' //            'INTEGRATION, RESPECTIVELY.  YOU HAVE CALLED THE CODE ' // 'WITH  INFO(3) = ' // XERN1, 5, 1)            'WITH  INFO(3) = ' // XERN1, 5, 1)
         IDID = -33
      ENDIF
      !
      IF (INFO(4) .NE. 0 .AND. INFO(4) .NE. 1) THEN
         WRITE (XERN1, '(I8)') INFO(4)
         CALL XERMSG ('SLATEC', 'DDEBDF', 'INFO(4) MUST BE 0 OR 1 ' // 'INDICATING WHETHER OR NOT THE INTEGRATION INTERVAL IS ' //            'INDICATING WHETHER OR NOT THE INTEGRATION INTERVAL IS ' // 'TO BE RESTRICTED BY A POINT TSTOP.  YOU HAVE CALLED ' //            'TO BE RESTRICTED BY A POINT TSTOP.  YOU HAVE CALLED ' // 'THE CODE WITH INFO(4) = ' // XERN1, 14, 1)            'THE CODE WITH INFO(4) = ' // XERN1, 14, 1)
         IDID = -33
      ENDIF
      !
      IF (INFO(5) .NE. 0 .AND. INFO(5) .NE. 1) THEN
         WRITE (XERN1, '(I8)') INFO(5)
         CALL XERMSG ('SLATEC',  'DDEBDF', 'INFO(5) MUST BE 0 OR 1 ' // 'INDICATING WHETHER THE CODE IS TOLD TO FORM THE ' //            'INDICATING WHETHER THE CODE IS TOLD TO FORM THE ' // 'JACOBIAN MATRIX BY NUMERICAL DIFFERENCING OR YOU ' //            'JACOBIAN MATRIX BY NUMERICAL DIFFERENCING OR YOU ' // 'PROVIDE A SUBROUTINE TO EVALUATE IT ANALYTICALLY.  ' //            'PROVIDE A SUBROUTINE TO EVALUATE IT ANALYTICALLY.  ' // 'YOU HAVE CALLED THE CODE WITH INFO(5) = ' // XERN1, 15, 1)            'YOU HAVE CALLED THE CODE WITH INFO(5) = ' // XERN1, 15, 1)
         IDID = -33
      ENDIF
      !
      IF (INFO(6) .NE. 0 .AND. INFO(6) .NE. 1) THEN
         WRITE (XERN1, '(I8)') INFO(6)
         CALL XERMSG ('SLATEC', 'DDEBDF', 'INFO(6) MUST BE 0 OR 1 ' // 'INDICATING WHETHER THE CODE IS TOLD TO TREAT THE ' //            'INDICATING WHETHER THE CODE IS TOLD TO TREAT THE ' // 'JACOBIAN AS A FULL (DENSE) MATRIX OR AS HAVING A ' //            'JACOBIAN AS A FULL (DENSE) MATRIX OR AS HAVING A ' // 'SPECIAL BANDED STRUCTURE.  YOU HAVE CALLED THE CODE ' //            'SPECIAL BANDED STRUCTURE.  YOU HAVE CALLED THE CODE ' // 'WITH INFO(6) = ' // XERN1, 16, 1)            'WITH INFO(6) = ' // XERN1, 16, 1)
         IDID = -33
      ENDIF
      !
      ILRW = NEQ
      IF (INFO(6) .NE. 0) THEN
         ...
         IF (ML.LT.0 .OR. ML.GE.NEQ .OR. MU.LT.0 .OR. MU.GE.NEQ) THEN
            WRITE (XERN1, '(I8)') ML
            WRITE (XERN2, '(I8)') MU
            CALL XERMSG ('SLATEC', 'DDEBDF', 'YOU HAVE SET INFO(6) ' // '= 1, TELLING THE CODE THAT THE JACOBIAN MATRIX HAS ' //               '= 1, TELLING THE CODE THAT THE JACOBIAN MATRIX HAS ' // 'A SPECIAL BANDED STRUCTURE.  HOWEVER, THE LOWER ' //               'A SPECIAL BANDED STRUCTURE.  HOWEVER, THE LOWER ' // '(UPPER) BANDWIDTHS  ML (MU) VIOLATE THE CONSTRAINTS ' //               '(UPPER) BANDWIDTHS  ML (MU) VIOLATE THE CONSTRAINTS ' // 'ML,MU .GE. 0 AND  ML,MU .LT. NEQ.  YOU HAVE CALLED ' //               'ML,MU .GE. 0 AND  ML,MU .LT. NEQ.  YOU HAVE CALLED ' // 'THE CODE WITH ML = ' // XERN1 // ' AND MU = ' // XERN2,               'THE CODE WITH ML = ' // XERN1 // ' AND MU = ' // XERN2,&
               17, 1)
            IDID = -33
         ENDIF
      ENDIF
      ...
      IF (LRW .LT. 250 + (10 + ILRW)*NEQ) THEN
         WRITE (XERN1, '(I8)') LRW
         IF (INFO(6) .EQ. 0) THEN
            CALL XERMSG ('SLATEC', 'DDEBDF', 'LENGTH OF ARRAY RWORK ' // 'MUST BE AT LEAST 250 + 10*NEQ + NEQ*NEQ.$$' //               'MUST BE AT LEAST 250 + 10*NEQ + NEQ*NEQ.$$' // 'YOU HAVE CALLED THE CODE WITH  LRW = ' // XERN1, 1, 1)               'YOU HAVE CALLED THE CODE WITH  LRW = ' // XERN1, 1, 1)
         ELSE
            CALL XERMSG ('SLATEC', 'DDEBDF', 'LENGTH OF ARRAY RWORK ' // 'MUST BE AT LEAST 250 + 10*NEQ + (2*ML+MU+1)*NEQ.$$' //               'MUST BE AT LEAST 250 + 10*NEQ + (2*ML+MU+1)*NEQ.$$' // 'YOU HAVE CALLED THE CODE WITH  LRW = ' // XERN1, 18, 1)               'YOU HAVE CALLED THE CODE WITH  LRW = ' // XERN1, 18, 1)
         ENDIF
         IDID = -33
      ENDIF
      !
      IF (LIW .LT. 56 + NEQ) THEN
         WRITE (XERN1, '(I8)') LIW
         CALL XERMSG ('SLATEC', 'DDEBDF', 'LENGTH OF ARRAY IWORK ' // 'BE AT LEAST  56 + NEQ.  YOU HAVE CALLED THE CODE WITH ' //            'BE AT LEAST  56 + NEQ.  YOU HAVE CALLED THE CODE WITH ' // 'LIW = ' // XERN1, 2, 1)            'LIW = ' // XERN1, 2, 1)
         IDID = -33
      ENDIF
      !
      ...
      IF (NEQ .LT. 1) THEN
         WRITE (XERN1, '(I8)') NEQ
         CALL XERMSG ('SLATEC', 'DLSOD', 'IN DDEBDF, THE NUMBER OF EQUATIONS MUST BE A ' //            'IN DDEBDF, THE NUMBER OF EQUATIONS MUST BE A ' // 'POSITIVE INTEGER.$$YOU HAVE CALLED THE CODE WITH NEQ = ' //            'POSITIVE INTEGER.$$YOU HAVE CALLED THE CODE WITH NEQ = ' //&
            XERN1, 6, 1)
         IDID=-33
      ENDIF
      !
      ...
      DO 60 K = 1, NEQ
         IF (NRTOLP .LE. 0) THEN
            IF (RTOL(K) .LT. 0.) THEN
               ...
               CALL XERMSG ('SLATEC', 'DLSOD', 'IN DDEBDF, THE RELATIVE ERROR TOLERANCES MUST ' //                  'IN DDEBDF, THE RELATIVE ERROR TOLERANCES MUST ' // 'BE NON-NEGATIVE.$$YOU HAVE CALLED THE CODE WITH ' //                  'BE NON-NEGATIVE.$$YOU HAVE CALLED THE CODE WITH ' // 'RTOL(' // XERN1 // ') = ' // XERN3 // '$$IN THE ' //                  'RTOL(' // XERN1 // ') = ' // XERN3 // '$$IN THE ' // 'CASE OF VECTOR ERROR TOLERANCES, NO FURTHER ' //                  'CASE OF VECTOR ERROR TOLERANCES, NO FURTHER ' // 'CHECKING OF RTOL COMPONENTS IS DONE.', 7, 1)                  'CHECKING OF RTOL COMPONENTS IS DONE.', 7, 1)
               IDID = -33
         ...
         IF (ATOL(K) .LT. 0.) THEN
            ...
            CALL XERMSG ('SLATEC', 'DLSOD', 'IN DDEBDF, THE ABSOLUTE ERROR ' //               'IN DDEBDF, THE ABSOLUTE ERROR ' // 'TOLERANCES MUST BE NON-NEGATIVE.$$YOU HAVE CALLED ' //               'TOLERANCES MUST BE NON-NEGATIVE.$$YOU HAVE CALLED ' // 'THE CODE WITH ATOL(' // XERN1 // ') = ' // XERN3 //               'THE CODE WITH ATOL(' // XERN1 // ') = ' // XERN3 // '$$IN THE CASE OF VECTOR ERROR TOLERANCES, NO FURTHER '               '$$IN THE CASE OF VECTOR ERROR TOLERANCES, NO FURTHER '&
   ...
   70 IF (ITSTOP .EQ. 1) THEN
         IF (SIGN(1.0D0,TOUT-T) .NE. SIGN(1.0D0,TSTOP-T) .OR.&
            ...
            CALL XERMSG ('SLATEC', 'DLSOD', 'IN DDEBDF, YOU HAVE CALLED THE ' //               'IN DDEBDF, YOU HAVE CALLED THE ' // 'CODE WITH TOUT = ' // XERN3 // '$$BUT YOU HAVE ' //               'CODE WITH TOUT = ' // XERN3 // '$$BUT YOU HAVE ' // 'ALSO TOLD THE CODE NOT TO INTEGRATE PAST THE POINT ' //               'ALSO TOLD THE CODE NOT TO INTEGRATE PAST THE POINT ' // 'TSTOP = ' // XERN4 // ' BY SETTING INFO(4) = 1.$$' //               'TSTOP = ' // XERN4 // ' BY SETTING INFO(4) = 1.$$' // 'THESE INSTRUCTIONS CONFLICT.', 14, 1)               'THESE INSTRUCTIONS CONFLICT.', 14, 1)
            IDID=-33
         ENDIF
      ENDIF
      !
      ...
      IF (INIT .NE. 0) THEN
         IF (T .EQ. TOUT) THEN
            WRITE (XERN3, '(1PE15.6)') T
            CALL XERMSG ('SLATEC', 'DLSOD', 'IN DDEBDF, YOU HAVE CALLED THE CODE WITH T = TOUT = ' //               'IN DDEBDF, YOU HAVE CALLED THE CODE WITH T = TOUT = ' //&
         ...
         IF (T .NE. TOLD) THEN
            ...
            CALL XERMSG ('SLATEC', 'DLSOD', 'IN DDEBDF, YOU HAVE CHANGED THE VALUE OF T FROM ' //               'IN DDEBDF, YOU HAVE CHANGED THE VALUE OF T FROM ' //&
               XERN3 // ' TO ' // XERN4 // '  THIS IS NOT ALLOWED ON CONTINUATION CALLS.', 10, 1)               '  THIS IS NOT ALLOWED ON CONTINUATION CALLS.', 10, 1)
            IDID=-33
         ENDIF
         !
         IF (INIT .NE. 1) THEN
            IF (DELSGN*(TOUT-T) .LT. 0.0D0) THEN
               WRITE (XERN3, '(1PE15.6)') TOUT
               CALL XERMSG ('SLATEC', 'DLSOD', 'IN DDEBDF, BY CALLING THE CODE WITH TOUT = ' //                  'IN DDEBDF, BY CALLING THE CODE WITH TOUT = ' //&
                  XERN3 // ' YOU ARE ATTEMPTING TO CHANGE THE ' // 'DIRECTION OF INTEGRATION.$$THIS IS NOT ALLOWED ' //                  'DIRECTION OF INTEGRATION.$$THIS IS NOT ALLOWED ' // 'WITHOUT RESTARTING.', 11, 1)                  'WITHOUT RESTARTING.', 11, 1)
               IDID=-33
            ENDIF
         ENDIF
      ENDIF
      ...
      IF (IDID .EQ. (-33)) THEN
         IF (IQUIT .NE. (-33)) THEN
            ...
            IBEGIN=-1
         ELSE
            CALL XERMSG ('SLATEC', 'DLSOD', 'IN DDEBDF, INVALID INPUT WAS DETECTED ON ' //               'IN DDEBDF, INVALID INPUT WAS DETECTED ON ' // 'SUCCESSIVE ENTRIES.  IT IS IMPOSSIBLE TO PROCEED ' //               'SUCCESSIVE ENTRIES.  IT IS IMPOSSIBLE TO PROCEED ' // 'BECAUSE YOU HAVE NOT CORRECTED THE PROBLEM, ' //               'BECAUSE YOU HAVE NOT CORRECTED THE PROBLEM, ' // 'SO EXECUTION IS BEING TERMINATED.', 12, 2)               'SO EXECUTION IS BEING TERMINATED.', 12, 2)
         ENDIF
         RETURN
      ENDIF
      !
