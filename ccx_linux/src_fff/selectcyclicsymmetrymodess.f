...
!
      ...
      do i=2,n
         if(textpart(i)(1:5).eq.'NMIN=') then
            ...
            if(istat.gt.0) then
               call inputerror(inpc,ipoinpc,iline, "*SELECT CYCLIC SYMMETRY MODES%",ier)                    "*SELECT CYCLIC SYMMETRY MODES%",ier)
               return
            endif
         elseif(textpart(i)(1:5).eq.'NMAX=') then
            ...
            call inputwarning(inpc,ipoinpc,iline, "*SELECT CYCLIC SYMMETRY MODES%")      "*SELECT CYCLIC SYMMETRY MODES%")
         endif
      enddo
      !
      !     check the input
            ...
            if(node.lt.0) then
               ...
               if(ns(2).eq.0) then
                  ...
                  do k=1,2
                     ...
                     if(id.gt.0) then
                        if(ikmpc(id).eq.idof) then
                           write(*,*) '*ERROR reading *SELECT CYCLIC SYMMETRY MODES:'                       '*ERROR reading *SELECT CYCLIC SYMMETRY MODES:'
                           write(*,*) '       node',node, ' on cyclic symmetry'                                ' on cyclic symmetry'
