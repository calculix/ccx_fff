...
!
      ...
      elseif((iperturb(1).eq.1).and.(istep.gt.1)) then
         write(*,*) '*ERROR reading *UNCOUPLED TEMPERATURE-DISPLACEMENT:'             '*ERROR reading *UNCOUPLED TEMPERATURE-DISPLACEMENT:'
      ...
      do i=2,n
         if(textpart(i)(1:6).eq.'ALPHA=') then
            ...
            if(istat.gt.0) then
               call inputerror(inpc,ipoinpc,iline, "*UNCOUPLED TEMPERATURE-DISPLACEMENT%",ier)                  "*UNCOUPLED TEMPERATURE-DISPLACEMENT%",ier)
               return
            endif
         ...
         elseif(textpart(i)(1:17).eq.'TOTALTIMEATSTART=') then
            ...
            write(*,*) '*WARNING in uncouptempdisps: parameter not recognized:'              '*WARNING in uncouptempdisps: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*UNCOUPLED TEMPERATURE-DISPLACEMENT%")      "*UNCOUPLED TEMPERATURE-DISPLACEMENT%")
         endif
      enddo
      ...
      elseif(solver(1:6).eq.'PASTIX') then
         ...
         write(*,*) '*WARNING reading *UNCOUPLED TEMPERATURE-DISPLACEMENT:'             '*WARNING reading *UNCOUPLED TEMPERATURE-DISPLACEMENT:'
