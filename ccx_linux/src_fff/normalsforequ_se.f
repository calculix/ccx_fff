         ...
         if(nodedesiinv(i).eq.0) then   
            ...
            if((lakon(ielem)(7:7).eq.'A').or.&
               ...
               if(lakon(ielem)(4:5).eq.'20') then
                  ...
                  !
                  !                   write(*,*) 'normalsforequ_se ',i,iponoelfa(i)
                  !                 write(*,*) 'normalsforequ_se ',i,inoelfa(1,iponoelfa(i))
                  !           write(*,*) 'normalsforequ_se ',(konfa(ipkonfa(iface)+j),j=1,4)
                  !                   iface=inoelfa(1,iponoelfa(i))
                  !                   if(iface.le.2) cycle
         ...
         do
            ...
            if((lakon(ielem)(7:7).eq.'A').or.&
               ...
               if(iface.gt.2) then
                  ...
                  !                   do j=1,nope
                  !                      konl(j)=kon(ipkon(ielem)+j)
                  !                   enddo
                  ! !
                  !                   do j=1,nopem
                  !                      nopesurf(j)=konl(ifaceq(j,iface))
                  !                      do k=1,3
                  !                         xl(k,j)=co(k,nopesurf(j))
                  !                      enddo
                  !                   enddo
                  !                   xi=0.d0
                  !                   et=0.d0
                  !                   call norshell8(xi,et,xl,xno(1,1))
                  !                   dd=dsqrt(xno(1,1)**2+xno(2,1)**2+
                  !      &               xno(3,1)**2)
                  !                   if(dd.gt.0.d0) then
                  !                      do j=1,3
                  !                         xno(j,1)=xno(j,1)/dd
