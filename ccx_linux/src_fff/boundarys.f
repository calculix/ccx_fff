...
!
      ...
      do i=2,n
         ...
         elseif(textpart(i)(1:6).eq.'OP=NEW') then
               write(*,*) '*INFO reading *BOUNDARY: OP=NEW only makes sense'                 '*INFO reading *BOUNDARY: OP=NEW only makes sense'
               write(*,*) '      on the first *BOUNDARY card within a step'                    '      on the first *BOUNDARY card within a step'
               write(*,*)
         elseif(textpart(i)(1:10).eq.'AMPLITUDE=') then
            ...
            enddo
            if(j.eq.0) then
               write(*,*) '*ERROR reading *BOUNDARY: nonexistent amplitude'                 '*ERROR reading *BOUNDARY: nonexistent amplitude'
               write(*,*) '  '
         ...
         elseif(textpart(i)(1:8).eq.'DATASET=') then
            ...
            write(*,*) '*WARNING reading *BOUNDARY: parameter not recognized:'              '*WARNING reading *BOUNDARY: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*BOUNDARY%")      "*BOUNDARY%")
         endif
      enddo
      !
                  ...
                  if(submodel) then
                     if(ntrans.gt.0) then
                        if(inotr(1,k).gt.0) then
                           write(*,*) '*ERROR reading *BOUNDARY: in submodel'                             '*ERROR reading *BOUNDARY: in submodel'
                           write(*,*) '       node',k, ' a local coordinate'                             ' a local coordinate'
                           write(*,*) '       system was defined. This is not'                             '       system was defined. This is not'
                           write(*,*) '       allowed'
                           ier=1
                           return
                        endif
