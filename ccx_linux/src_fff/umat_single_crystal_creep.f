...
!
      ...
      !     18. n=0,0,1   l=1,-1,0
      !
      xm=reshape((/0.4082482904639d+00,-0.4082482904639d+00, 0.0000000000000d+00,&
          !&
         ...
         -0.2041241452319d+00, 0.0000000000000d+00,-0.2041241452319d+00,&
          ...
          0.0000000000000d+00, 0.0000000000000d+00, 0.0000000000000d+00,&
          0.0000000000000d+00, 0.3535533905933d+00,-0.3535533905933d+00/),(/6,18/))
      !
      !
      xx=reshape((/0.4082482904639d+00,-0.4082482904639d+00, 0.0000000000000d+00,&
          !&
          0.0000000000000d+00, 0.2041241452319d+00,-0.2041241452319d+00,&
          0.4082482904639d+00, 0.0000000000000d+00,-0.4082482904639d+00,&
          0.2041241452319d+00, 0.0000000000000d+00,-0.2041241452319d+00,&
      ...
      do i=1,18
         dg(i)=0.d0
      !          dg(i)=dtime*htri(i)**cn(i)/ck(i)**cn(i)
      enddo
      !
      !     major loop
      !
      ...
      do
         ...
         if(icounter.gt.100) then
            ...
            return
         !             write(*,*)
         !      &       '*ERROR in umat_single_crystal_creep: no convergence'
         !             call exit(201)
         endif
         !
         !     elastic strains
         ...
         do i=1,6
            ee(i)=emec(i)-ep(i)
         !          write(*,*) 'umat_single_crystal_creep ee',icounter,i,ee(i)
         enddo
         !
         !     global trial stress tensor
         !
         ...
         do i=1,6
         !          write(*,*) 'umat_single_crystal_creep stri',icounter,i,stri(i)
         enddo
         !
         !     stress radius in each slip plane
         !
         do i=1,18
            sg(i)=xm(1,i)*stri(1)+xm(2,i)*stri(2)+xm(3,i)*stri(3)+&
                 2.d0*(xm(4,i)*stri(4)+xm(5,i)*stri(5)+xm(6,i)*stri(6))
         !          write(*,*) 'umat_single_crystal_creep sg',icounter,i,sg(i)
         enddo
         !
         !     evaluation of the yield surface
         !
         do i=1,18
            !          write(*,*) 'umat_single_crystal_creep ck,dtime,cn,dg',icounter,
            !      &         i,ck(i),dtime,cn(i),dg(i)
            htri(i)=dabs(sg(i))-ck(i)*(dg(i)/dtime)**(1.d0/cn(i))
         !          write(*,*) 'umat_single_crystal_creep htri',icounter,i,htri(i)
         enddo
         !
         ...
         if(icounter.gt.1) then
            convergence=1
            do i=1,18
               !                if(dabs(htri(i)).gt.1.d-10) then
               if(dabs(htri(i)).gt.1.d-3) then
                  !                   write(*,*) 'divergence htri dg ',
                  !      &                    icounter,i,htri(i),dg(i)
                  convergence=0
                  exit
               endif
            enddo
         ...
         do i=1,18
            !
            gr(i,1)=htri(i)
            ! ccccc     &           -ck(i)*(dg(i)/dtime)**(1.d0/cn(i))
            !
            do j=1,6
               t(j)=xmc(j,i)*sg(i)
            enddo
         ...
         do j=1,6
            ...
            enddo
         !          write(*,*) 'umat_single_crystal_creep ep',icounter,j,ep(j)
         enddo
         !
         ...
         !          write(*,*) 'umat_single_crystal_creep ddg',icounter,i,ddg(i)
         enddo
      !          write(*,*) 'umat_single_crystal_creep ',icounter,dg(2)
      !
      !     end of major loop
      !
      enddo
