...
!
      ...
      !
      if(istep.gt.0) then
         write(*,*) '*ERROR reading *ORIENTATION: *ORIENTATION should be'             '*ERROR reading *ORIENTATION: *ORIENTATION should be'
      ...
      do i=2,n
         ...
         elseif(textpart(i)(1:7).eq.'SYSTEM=') then
            ...
            write(*,*) '*WARNING reading *ORIENTATION: parameter not recognized:'              '*WARNING reading *ORIENTATION: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*ORIENTATION%")      "*ORIENTATION%")
         endif
      enddo
      !
      ...
      if((istat.lt.0).or.(key.eq.1)) then
         write(*,*) '*ERROR reading *ORIENTATION: definition of the following'            '*ERROR reading *ORIENTATION: definition of the following'
      ...
      if((istat.ge.0).and.(key.ne.1)) then
         ...
         if(istat.gt.0) then
            write(*,*) '*ERROR reading *ORIENTATION: expected', 'axis of rotation'              'axis of rotation'
            call inputerror(inpc,ipoinpc,iline, "*ORIENTATION%",ier)                 "*ORIENTATION%",ier)
            return
         endif
         read(textpart(2)(1:20),'(f20.0)',iostat=istat) angle
         if(istat.gt.0) then
            write(*,*) '*ERROR reading *ORIENTATION: expected', 'angle of rotation'              'angle of rotation'
            call inputerror(inpc,ipoinpc,iline, "*ORIENTATION%",ier)                 "*ORIENTATION%",ier)
            return
         endif
         call getnewline(inpc,textpart,istat,n,key,iline,ipol,inl,&
            ...
            if(orname(iorien).eq.distname) then
               ...
               if(iaxis.ne.0) then
                  ...
                  if(orab(7,iorien).lt.0.d0) then
                     write(*,*) '*ERROR reading *ORIENTATION'
                     write(*,*) '       additional rotation about an angle'                      '       additional rotation about an angle'
                     write(*,*) '       is only allowed for rectangular systems'                      '       is only allowed for rectangular systems'
                     ier=1
                     return
                  endif
                  !
