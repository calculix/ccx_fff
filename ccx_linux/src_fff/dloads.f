      ...
      do i=2,n
        ...
        elseif(textpart(i)(1:10).eq.'AMPLITUDE=') then
          ...
          if(j.gt.nam) then
            ...
            write(*,*) '  '
            call inputerror(inpc,ipoinpc,iline, "*DLOAD%",ier)                 "*DLOAD%",ier)
            return
          endif
          iamplitude=j
        elseif(textpart(i)(1:10).eq.'TIMEDELAY=') THEN
          if(idelay.ne.0) then
            write(*,*) '*ERROR reading *DLOAD: the parameter TIME DELAY'                 '*ERROR reading *DLOAD: the parameter TIME DELAY'
        ...
        elseif(textpart(i)(1:9).eq.'LOADCASE=') then
          ...
          if(nmethod.ne.5) then
            write(*,*) '*ERROR reading *DLOAD: the parameter LOAD CASE'                 '*ERROR reading *DLOAD: the parameter LOAD CASE'
        ...
        elseif(textpart(i)(1:8).eq.'DATASET=') then
          ...
          write(*,*) '*WARNING reading *DLOAD: parameter not recognized:'               '*WARNING reading *DLOAD: parameter not recognized:'
          write(*,*) '         ',&
               textpart(i)(1:index(textpart(i),' ')-1)
          call inputwarning(inpc,ipoinpc,iline, "*DLOAD%")               "*DLOAD%")
        endif
      enddo
      !
