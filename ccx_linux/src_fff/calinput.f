...
!
      ...
      !
      elseif(textpart(1)(1:34).eq. '*UNCOUPLEDTEMPERATURE-DISPLACEMENT') then             '*UNCOUPLEDTEMPERATURE-DISPLACEMENT') then
      ...
      preted:'
        call inputwarning(inpc,ipoinpc,iline, "the input file%")             "the input file%")
        call getnewline(inpc,textpart,istat,n,key,iline,ipol,inl,&
             ipoinp,inp,ipoinpc)
      endif
      !
      ...
      if((ithermal(2).le.1).and.(nmethod.le.7)) then
        ...
        do i=1,nprint
          ...
          elseif(prlab(i)(1:4).eq.'TS  ') then
            write(*,*) '*WARNING in calinput: total temperature output'                 '*WARNING in calinput: total temperature output'
            write(*,*) '         requested, yet no heat transfer'
            write(*,*) '         calculation'
            write(*,*)
            cycle
      ...
      if(((nbody.gt.0).or.&
        ...
        do i=1,nmat
          if((nrhcon(i).ne.0).or.(matname(i)(1:6).eq.'SPRING').or.&
            ...
            write(*,*) '         to material ',&
                 matname(i)(1:index(matname(i),' ')-1), ' in a dynamic'                 ' in a dynamic'
        ...
        if(ierror.eq.0) then
          ...
          write(*,*) '       to any material ', ' in a dynamic'               ' in a dynamic'
          write(*,*) '       calculation or a calculation with'
          write(*,*) '       centrifugal or gravitational loads'
          call exit(201)
        endif
      ...
      if((nmethod.eq.2).or.(nmethod.eq.4)) then
        if(ithermal(1).ge.2) then
          ...
          do i=1,nmat
            if(nshcon(i).ne.0) then
              ...
              write(*,*) '         was assigned to material ',&
                   matname(i)(1:index(matname(i),' ')-1), ' in a transient'                   ' in a transient'
          ...
          if(ierror.eq.0) then
            ...
            write(*,*) '       assigned to any material ', ' in a transient'                 ' in a transient'
            write(*,*) '       heat transfer calculation'
            write(*,*)
            call exit(201)
          endif
      ...
      if((ithermal(1).ge.2).and.(nef.eq.0)) then
        ...
        do i=1,nmat
          if(ncocon(1,i).ne.0) then
            ...
            write(*,*) '         constants were assigned to material ',                 '         constants were assigned to material ',&
                 matname(i)(1:index(matname(i),' ')-1)
            write(*,*) '         in a thermo(mechanical) calculation'
            write(*,*)
          endif
