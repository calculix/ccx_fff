...
!
      ...
      integer mi(*),istartset(*),iendset(*),ialset(*),nconstants,&
        ...
        nodea,nodeb,noil_mat,npu,nfix,nstart,iset,ier,ndpropread
      !
      real*8 prop(*)
      ...
      if((istep.gt.0).and.(irstrt(1).ge.0)) then
         write(*,*) '*ERROR reading *FLUID SECTION: *FLUID SECTION should'            '*ERROR reading *FLUID SECTION: *FLUID SECTION should'
      ...
      do i=2,n
         ...
         elseif(textpart(i)(1:4).eq.'OIL=') then
            ...
            enddo
            if(j.gt.nmat) then
               write(*,*) '*ERROR reading *FLUID SECTION: no oil with the'                  '*ERROR reading *FLUID SECTION: no oil with the'
         ...
         elseif(textpart(i)(1:10).eq.'CONSTANTS=') then
            ...
            write(*,*) '*WARNING reading *FLUID SECTION: parameter not recognized:'            '*WARNING reading *FLUID SECTION: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*FLUID SECTION%")      "*FLUID SECTION%")
         endif
      enddo
      !
      !     types of sections
      ...
      enddo
      if(i.gt.nmat) then
         write(*,*) '*ERROR reading *FLUID SECTION: nonexistent material'           '*ERROR reading *FLUID SECTION: nonexistent material'
         write(*,*) '  '
         call inputerror(inpc,ipoinpc,iline, "*FLUID SECTION%",ier)              "*FLUID SECTION%",ier)
         return
      endif
            ...
            do j=1,n
               ...
               elseif(elname(1:4).eq.'CHAR') then
                  if(ndprop.gt.65) then
                     ...
                     write(*,*) '      fluid section type CHARACTERISTIC'                    '      fluid section type CHARACTERISTIC'
                     ier=1
                     return
                  endif
               elseif(elname(1:4).eq.'ORPN') then
                  if(ndprop.gt.41) then
                     ...
                     write(*,*) '      fluid section type PRESWIRL NOZZLE'                    '      fluid section type PRESWIRL NOZZLE'
                     ier=1
                     return
                  endif
               elseif(elname(1:4).eq.'ORBT') then
      ...
      do j=istartset(iset),iendset(iset)
         if(ialset(j).gt.0) then
            ...
            if(liquid.and.((lakon(ialset(j))(4:5).eq.'BG').or.&
               ...
               write(*,*) ''
               write(*,*) '*ERROR reading *FLUID SECTION: element ',k, ' is no valid incompressible orifice element.'      ' is no valid incompressible orifice element.'
               write(*,*) ' Please change element type', ' and/or definition.'      ' and/or definition.'
