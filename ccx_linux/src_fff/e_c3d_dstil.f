...
!
         ...
         if(islavelinv(nelem).gt.0)then
            if(nope.eq.20 .or. nope.eq.10 .or. nope.eq.15)then
               do i1=1,nope
                  ...
                  enddo
               !      write(*,*) i1,'   shp',shp(4,i1),shptil(4,i1),
               !      &      shptil(4,i1)-shp(4,i1)
               enddo
 ...
 !
 !       if(nelem.eq.1) then
 !          open(30,file='nelem1_dstil.dat',status='unknown')
 !          write(30,'(8(1x,e11.4))') ((s(i,j),i=1,60),j=1,60)
 !          close(30)
 !       elseif(nelem.eq.2) then
 !          open(31,file='nelem2.dat',status='unknown')
 !          write(31,'(8(1x,e11.4))') ((s(i,j),i=1,60),j=1,60)
 !          close(31)
 !       elseif(nelem.eq.3) then
 !          open(32,file='nelem3.dat',status='unknown')
 !          write(32,'(8(1x,e11.4))') ((s(i,j),i=1,60),j=1,60)
 !          close(32)
 !       else
 !          open(33,file='nelem4.dat',status='unknown')
 !          write(33,'(8(1x,e11.4))') ((s(i,j),i=1,60),j=1,60)
 !          close(33)
 !       endif
