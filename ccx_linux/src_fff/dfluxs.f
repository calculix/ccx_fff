      ...
      do i=2,n
        ...
        elseif(textpart(i)(1:10).eq.'AMPLITUDE=') then
          ...
          if(j.eq.0) then
            ...
            write(*,*) '  '
            call inputerror(inpc,ipoinpc,iline, "*DFLUX%",ier)                 "*DFLUX%",ier)
            return
          endif
          iamplitude=j
        elseif(textpart(i)(1:10).eq.'TIMEDELAY=') THEN
          if(idelay.ne.0) then
            write(*,*) '*ERROR reading *DFLUX: the parameter TIME DELAY'                 '*ERROR reading *DFLUX: the parameter TIME DELAY'
          ...
          write(*,*) '*WARNING reading *DFLUX: parameter not recognized:'               '*WARNING reading *DFLUX: parameter not recognized:'
          write(*,*) '         ',&
               textpart(i)(1:index(textpart(i),' ')-1)
          call inputwarning(inpc,ipoinpc,iline, "*DFLUX%")               "*DFLUX%")
        endif
      enddo
      !
      do
