...
!
      ...
      loop: do i=2,n
               ...
               if(set(js).eq.elset) then
                  ...
                  if(iendset(js).eq.nalset) then
                     ...
                     if(nalset+nn.gt.nalset_) then
                        write(*,*) '*ERROR reading *ELEMENT: increase nalset_'                         '*ERROR reading *ELEMENT: increase nalset_'
                        ier=1
                        return
                     endif
                     do k=1,nn
         ...
         elseif(textpart(i)(1:7).eq.'PARENT=') then
            ...
            write(*,*) '*WARNING reading *ELEMENT: parameter not recognized:'              '*WARNING reading *ELEMENT: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*ELEMENT%")      "*ELEMENT%")
         endif
      enddo loop
      !
      if(label.eq.'        ') then
      ...
      elseif(label(1:1).eq.'U') then
         number=ichar(label(2:2))*256**3+&
                ...
                ichar(label(4:4))*256+&
                ichar(label(5:5))
         !          read(label(2:3),'(i2)',iostat=istat) number
         ...
         if(nteller.lt.nope) then
            do
               call getnewline(inpc,textpart,istat,n,key,iline,ipol,inl,&
                    ipoinp,inp,ipoinpc)
               if((istat.lt.0).or.(key.eq.1)) then
                  write(*,*) '*ERROR reading *ELEMENT: element definition'                    '*ERROR reading *ELEMENT: element definition'
