...
!
      ...
      !
      do m=2,n
         write(*,*) '*WARNING reading *EQUATIONF: parameter not recognized:'              '*WARNING reading *EQUATIONF: parameter not recognized:'
         write(*,*) '         ',&
              textpart(m)(1:index(textpart(m),' ')-1)
         call inputwarning(inpc,ipoinpc,iline, "*EQUATIONF%")      "*EQUATIONF%")
      enddo
      !
      if(istep.gt.0) then
         write(*,*) '*ERROR reading *EQUATIONF: *EQUATIONF should be placed'           '*ERROR reading *EQUATIONF: *EQUATIONF should be placed'
      ...
      do
         ...
         do
            ...
            if((istat.lt.0).or.(key.eq.1)) then
               ...
               call inputerror(inpc,ipoinpc,iline, "*EQUATIONF%",ier)                    "*EQUATIONF%",ier)
               return
            endif
            !
            do i=1,n/4
               ...
               if((itr.eq.0).or.(ndir.eq.0).or.(ndir.eq.4)) then
                  ...
                  if(mpcfree.eq.0) then
                     write(*,*) '*ERROR reading *EQUATIONF: increase memmpc_'                       '*ERROR reading *EQUATIONF: increase memmpc_'
                     ier=1
                     return
                  endif
               else
                  ...
                  if(ii.eq.0) then
                     ...
                     enddo
                     if(j.gt.3) then
                        write(*,*) '*ERROR reading *EQUATIONF: MPC on face'                             '*ERROR reading *EQUATIONF: MPC on face'
