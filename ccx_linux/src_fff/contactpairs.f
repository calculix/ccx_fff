...
!
      ...
      do i=2,n
         ...
         elseif(textpart(i)(1:7).eq.'ADJUST=') then
            ...
            if(istat.gt.0) then
               ...
               if(j.gt.nset) then
                  noset(ipos:ipos)=' '
                  write(*,*) '*ERROR reading *CONTACT PAIR: adjust node set',                     '*ERROR reading *CONTACT PAIR: adjust node set',&
                         noset
                  write(*,*) '       has not been defined'
                  call inputerror(inpc,ipoinpc,iline, "*CONTACT PAIR%",ier)                       "*CONTACT PAIR%",ier)
                  return
               endif
               do j=1,ipos-1
         ...
         elseif(textpart(i)(1:16).eq.'TYPE=PGLINMORTAR') then
            ...
            write(*,*) '*WARNING reading *CONTACT PAIR: parameter not recognized:'             '*WARNING reading *CONTACT PAIR: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*CONTACT PAIR%")      "*CONTACT PAIR%")
         endif
      enddo
      !
      if(mortar.lt.0) then
      ...
      !
      if(ncmat_<2) then
         write(*,*) '*ERROR reading *CONTACT PAIR: no PRESSURE-OVERCLOSURE'           '*ERROR reading *CONTACT PAIR: no PRESSURE-OVERCLOSURE'
         write(*,*) '       has been defined for at least one *SURFACE INTERACTION'         '       has been defined for at least one *SURFACE INTERACTION'
         ier=1
         return
