...
!
      ...
      if(vj.gt.1.d-30) then
         ...
         ! cc         write(*,*) '*WARNING in incplas: deformation inside-out'
         !
      ...
      if(dxitril.lt.0.d0) then
         ! cc         write(*,*) '*WARNING in incplas: dxitril < 0'
         dxitril=0.d0
      else
         ...
         if((kode.eq.-51).or.(ivisco.eq.0)) then
            ...
            if(user_creep.eq.1) then
               if(ithermal(1).eq.0) then
                  ! cc                  write(*,*) '*ERROR in incplas: no temperature defined'
                  call exit(201)
               endif
               timeabq(1)=time
         ...
         if((iloop.gt.15).or.(cop.le.0.d0)) then
            ...
            do
               if(iloop.gt.100) then
                  !                   NOTE: write statements cause problems for
                  !                         parallellized execution
                  !                   write(*,*) '*WARNING in incplas: material loop'
                  !                   write(*,*) '         did not converge in integration'
                  !                   write(*,*) '         point',iint,'in element',iel,';'
                  !                   write(*,*) '         the increment size is reduced'
                  !                   write(*,*)
                  pnewdt=0.25d0
                  return
               endif
