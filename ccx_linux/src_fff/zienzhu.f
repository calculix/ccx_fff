      ...
      patches:do nodebase=1,nk
         ...
         elseif(itypflag.eq.1.or.itypflag.eq.3) then
            ...
            if(&
               ...
               !                write(*,*) 'Node',nodebase,' is not valid as',
               !      &              ' a patch base node. Seeking another',
               !      &              ' patch base node.'
               !
               ...
               if(&
                  ...
                  write(*,*) '*WARNING in estimator: Patch not', ' appropriate for patch recovery,'                       ' appropriate for patch recovery,'
                  write(*,*) '                       using', ' average of sampling point values', nodebase                       ' average of sampling point values', nodebase
                  iavflag=1
               endif
            !
            !                write(*,*) 'Alternative node is',idxnode
            endif
            !
            index=ipneigh(idxnode)
            !
            ...
            if(itypflag.eq.1) then
               ...
               hexelements: do ielidx=1,linpatch
                  ...
                  hexnodes: do j=1,3
                     ...
                     k=k+1
                     if(k.gt.maxmid) then
                        write(*,*) '*ERROR in estimator: array size', ' for midnodes exceeded'                             ' for midnodes exceeded'
                        exit hexelements
                     endif
                  enddo hexnodes
               enddo hexelements
         ...
         elseif(itypflag.eq.2) then
            ...
            if(&
               ...
               if(icommon.gt.0) then
                  !                   write(*,*) 'conical patch found, node',nodebase,
                  !      &                 ' using node',idxnode,' as base node instead'
                  !
                  !                 count elements
                  !
                  index=ipneigh(idxnode)
            ...
            if(nelemv.gt.0.and.nelemv.lt.4) then
               !                write(*,*) 'node',nodebase,' has',nelemv,
               ...
               !                write(*,*) 'now node',nodebase,' has',nelemv,
               !      &              ' elements'
               !
               !              verify
               !
            ...
            !
            !             write(*,*) 'using',iterms,' terms for patch',nodebase
            !
