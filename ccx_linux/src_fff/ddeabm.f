      ...
      IF (IWORK(LIW) .GE. 5) THEN
         IF (T .EQ. RWORK(21 + NEQ)) THEN
            ...
            CALL XERMSG ('SLATEC', 'DDEABM', 'AN APPARENT INFINITE LOOP HAS BEEN DETECTED.$$' //               'AN APPARENT INFINITE LOOP HAS BEEN DETECTED.$$' // 'YOU HAVE MADE REPEATED CALLS AT T = ' // XERN3 //               'YOU HAVE MADE REPEATED CALLS AT T = ' // XERN3 // ' AND THE INTEGRATION HAS NOT ADVANCED.  CHECK THE ' //               ' AND THE INTEGRATION HAS NOT ADVANCED.  CHECK THE ' // 'WAY YOU HAVE SET PARAMETERS FOR THE CALL TO THE ' //               'WAY YOU HAVE SET PARAMETERS FOR THE CALL TO THE ' // 'CODE, PARTICULARLY INFO(1).', 13, 2)               'CODE, PARTICULARLY INFO(1).', 13, 2)
            RETURN
         ENDIF
      ENDIF
      !
      ...
      IF (LRW .LT. 130+21*NEQ) THEN
         WRITE (XERN1, '(I8)') LRW
         CALL XERMSG ('SLATEC', 'DDEABM', 'THE LENGTH OF THE RWORK ' // 'ARRAY MUST BE AT LEAST 130 + 21*NEQ.$$' //            'ARRAY MUST BE AT LEAST 130 + 21*NEQ.$$' // 'YOU HAVE CALLED THE CODE WITH LRW = ' // XERN1, 1, 1)            'YOU HAVE CALLED THE CODE WITH LRW = ' // XERN1, 1, 1)
         IDID=-33
      ENDIF
      !
      IF (LIW .LT. 51) THEN
         WRITE (XERN1, '(I8)') LIW
         CALL XERMSG ('SLATEC', 'DDEABM', 'THE LENGTH OF THE IWORK ' // 'ARRAY MUST BE AT LEAST 51.$$YOU HAVE CALLED THE CODE ' //            'ARRAY MUST BE AT LEAST 51.$$YOU HAVE CALLED THE CODE ' // 'WITH LIW = ' // XERN1, 2, 1)            'WITH LIW = ' // XERN1, 2, 1)
         IDID=-33
      ENDIF
      !
 ...
 50   CALL DDES(DF,NEQ,T,Y,TOUT,INFO,RTOL,ATOL,IDID,RWORK(IYPOUT),&
      ...
      IF (INFO(1) .NE. 0  .AND.  INFO(1) .NE. 1) THEN
         ...
         CALL XERMSG ('SLATEC', 'DDES', 'IN DDEABM, INFO(1) MUST BE ' // 'SET TO 0 FOR THE START OF A NEW PROBLEM, AND MUST BE ' //            'SET TO 0 FOR THE START OF A NEW PROBLEM, AND MUST BE ' // 'SET TO 1 FOLLOWING AN INTERRUPTED TASK.  YOU ARE ' //            'SET TO 1 FOLLOWING AN INTERRUPTED TASK.  YOU ARE ' // 'ATTEMPTING TO CONTINUE THE INTEGRATION ILLEGALLY BY ' //            'ATTEMPTING TO CONTINUE THE INTEGRATION ILLEGALLY BY ' // 'CALLING THE CODE WITH INFO(1) = ' // XERN1, 3, 1)            'CALLING THE CODE WITH INFO(1) = ' // XERN1, 3, 1)
         IDID=-33
      ENDIF
      !
      IF (INFO(2) .NE. 0  .AND.  INFO(2) .NE. 1) THEN
         WRITE (XERN1, '(I8)') INFO(2)
         CALL XERMSG ('SLATEC', 'DDES', 'IN DDEABM, INFO(2) MUST BE ' // '0 OR 1 INDICATING SCALAR AND VECTOR ERROR TOLERANCES, ' //            '0 OR 1 INDICATING SCALAR AND VECTOR ERROR TOLERANCES, ' // 'RESPECTIVELY.  YOU HAVE CALLED THE CODE WITH INFO(2) = ' //            'RESPECTIVELY.  YOU HAVE CALLED THE CODE WITH INFO(2) = ' //&
            XERN1, 4, 1)
         IDID=-33
      ENDIF
      !
      IF (INFO(3) .NE. 0  .AND.  INFO(3) .NE. 1) THEN
         ...
         CALL XERMSG ('SLATEC', 'DDES', 'IN DDEABM, INFO(3) MUST BE ' // '0 OR 1 INDICATING THE INTERVAL OR INTERMEDIATE-OUTPUT ' //            '0 OR 1 INDICATING THE INTERVAL OR INTERMEDIATE-OUTPUT ' // 'MODE OF INTEGRATION, RESPECTIVELY.  YOU HAVE CALLED ' //            'MODE OF INTEGRATION, RESPECTIVELY.  YOU HAVE CALLED ' // 'THE CODE WITH  INFO(3) = ' // XERN1, 5, 1)            'THE CODE WITH  INFO(3) = ' // XERN1, 5, 1)
         IDID=-33
      ENDIF
      !
      IF (INFO(4) .NE. 0  .AND.  INFO(4) .NE. 1) THEN
         WRITE (XERN1, '(I8)') INFO(4)
         CALL XERMSG ('SLATEC', 'DDES', 'IN DDEABM, INFO(4) MUST BE ' // '0 OR 1 INDICATING WHETHER OR NOT THE INTEGRATION ' //            '0 OR 1 INDICATING WHETHER OR NOT THE INTEGRATION ' // 'INTERVAL IS TO BE RESTRICTED BY A POINT TSTOP.  YOU ' //            'INTERVAL IS TO BE RESTRICTED BY A POINT TSTOP.  YOU ' // 'HAVE CALLED THE CODE WITH INFO(4) = ' // XERN1, 14, 1)            'HAVE CALLED THE CODE WITH INFO(4) = ' // XERN1, 14, 1)
         IDID=-33
      ENDIF
      !
      IF (NEQ .LT. 1) THEN
         WRITE (XERN1, '(I8)') NEQ
         CALL XERMSG ('SLATEC', 'DDES', 'IN DDEABM,  THE NUMBER OF ' // 'EQUATIONS NEQ MUST BE A POSITIVE INTEGER.  YOU HAVE ' //            'EQUATIONS NEQ MUST BE A POSITIVE INTEGER.  YOU HAVE ' // 'CALLED THE CODE WITH  NEQ = ' // XERN1, 6, 1)            'CALLED THE CODE WITH  NEQ = ' // XERN1, 6, 1)
         IDID=-33
      ENDIF
      !
      NRTOLP = 0
      ...
      DO 90 K=1,NEQ
         IF (NRTOLP .EQ. 0 .AND. RTOL(K) .LT. 0.D0) THEN
            ...
            CALL XERMSG ('SLATEC', 'DDES', 'IN DDEABM, THE RELATIVE ' // 'ERROR TOLERANCES RTOL MUST BE NON-NEGATIVE.  YOU ' //               'ERROR TOLERANCES RTOL MUST BE NON-NEGATIVE.  YOU ' // 'HAVE CALLED THE CODE WITH  RTOL(' // XERN1 // ') = ' //               'HAVE CALLED THE CODE WITH  RTOL(' // XERN1 // ') = ' //&
               XERN3 // '.  IN THE CASE OF VECTOR ERROR TOLERANCES, ' // 'NO FURTHER CHECKING OF RTOL COMPONENTS IS DONE.', 7, 1)               'NO FURTHER CHECKING OF RTOL COMPONENTS IS DONE.', 7, 1)
            IDID = -33
            NRTOLP = 1
         ENDIF
         !
         IF (NATOLP .EQ. 0 .AND. ATOL(K) .LT. 0.D0) THEN
            ...
            CALL XERMSG ('SLATEC', 'DDES', 'IN DDEABM, THE ABSOLUTE ' // 'ERROR TOLERANCES ATOL MUST BE NON-NEGATIVE.  YOU ' //               'ERROR TOLERANCES ATOL MUST BE NON-NEGATIVE.  YOU ' // 'HAVE CALLED THE CODE WITH  ATOL(' // XERN1 // ') = ' //               'HAVE CALLED THE CODE WITH  ATOL(' // XERN1 // ') = ' //&
               XERN3 // '.  IN THE CASE OF VECTOR ERROR TOLERANCES, ' // 'NO FURTHER CHECKING OF ATOL COMPONENTS IS DONE.', 8, 1)               'NO FURTHER CHECKING OF ATOL COMPONENTS IS DONE.', 8, 1)
            IDID = -33
            NATOLP = 1
         ENDIF
         !
  ...
  100 IF (INFO(4) .EQ. 1) THEN
         IF (SIGN(1.D0,TOUT-T) .NE. SIGN(1.D0,TSTOP-T)&
            ...
            CALL XERMSG ('SLATEC', 'DDES', 'IN DDEABM, YOU HAVE ' // 'CALLED THE CODE WITH  TOUT = ' // XERN3 // ' BUT ' //               'CALLED THE CODE WITH  TOUT = ' // XERN3 // ' BUT ' // 'YOU HAVE ALSO TOLD THE CODE (INFO(4) = 1) NOT TO ' //               'YOU HAVE ALSO TOLD THE CODE (INFO(4) = 1) NOT TO ' // 'INTEGRATE PAST THE POINT TSTOP = ' // XERN4 //               'INTEGRATE PAST THE POINT TSTOP = ' // XERN4 // ' THESE INSTRUCTIONS CONFLICT.', 14, 1)               ' THESE INSTRUCTIONS CONFLICT.', 14, 1)
            IDID=-33
         ENDIF
      ENDIF
      !
      ...
      IF (INIT .NE. 0) THEN
         IF (T .EQ. TOUT) THEN
            WRITE (XERN3, '(1PE15.6)') T
            CALL XERMSG ('SLATEC', 'DDES', 'IN DDEABM, YOU HAVE ' // 'CALLED THE CODE WITH  T = TOUT = ' // XERN3 //               'CALLED THE CODE WITH  T = TOUT = ' // XERN3 // '$$THIS IS NOT ALLOWED ON CONTINUATION CALLS.', 9, 1)               '$$THIS IS NOT ALLOWED ON CONTINUATION CALLS.', 9, 1)
            IDID=-33
         ENDIF
         !
         IF (T .NE. TOLD) THEN
            ...
            CALL XERMSG ('SLATEC', 'DDES', 'IN DDEABM, YOU HAVE ' // 'CHANGED THE VALUE OF T FROM ' // XERN3 // ' TO ' //               'CHANGED THE VALUE OF T FROM ' // XERN3 // ' TO ' //&
         ...
         IF (INIT .NE. 1) THEN
            IF (DELSGN*(TOUT-T) .LT. 0.D0) THEN
               WRITE (XERN3, '(1PE15.6)') TOUT
               CALL XERMSG ('SLATEC', 'DDES', 'IN DDEABM, BY ' // 'CALLING THE CODE WITH TOUT = ' // XERN3 //                  'CALLING THE CODE WITH TOUT = ' // XERN3 // ' YOU ARE ATTEMPTING TO CHANGE THE DIRECTION OF ' //                  ' YOU ARE ATTEMPTING TO CHANGE THE DIRECTION OF ' // 'INTEGRATION.$$THIS IS NOT ALLOWED WITHOUT ' //                  'INTEGRATION.$$THIS IS NOT ALLOWED WITHOUT ' // 'RESTARTING.', 11, 1)                  'RESTARTING.', 11, 1)
               IDID=-33
            ENDIF
         ENDIF
      ENDIF
      ...
      IF (IDID .EQ. (-33)) THEN
         IF (IQUIT .NE. (-33)) THEN
            ...
            INFO(1) = -1
         ELSE
            CALL XERMSG ('SLATEC', 'DDES', 'IN DDEABM, INVALID ' // 'INPUT WAS DETECTED ON SUCCESSIVE ENTRIES.  IT IS ' //               'INPUT WAS DETECTED ON SUCCESSIVE ENTRIES.  IT IS ' // 'IMPOSSIBLE TO PROCEED BECAUSE YOU HAVE NOT ' //               'IMPOSSIBLE TO PROCEED BECAUSE YOU HAVE NOT ' // 'CORRECTED THE PROBLEM, SO EXECUTION IS BEING ' //               'CORRECTED THE PROBLEM, SO EXECUTION IS BEING ' // 'TERMINATED.', 12, 2)               'TERMINATED.', 12, 2)
         ENDIF
         RETURN
      ENDIF
      !
 ...
 130    YPOUT(L) = HMU*(OY(L) - Y(L)) +&
      ...
      IF (NERR.LT.-9999999 .OR. NERR.GT.99999999 .OR. NERR.EQ.0 .OR.&
         ...
         CALL XERPRN (' ***', -1, 'FATAL ERROR IN...$$ ' // 'XERMSG -- INVALID ERROR NUMBER OR LEVEL$$ '//            'XERMSG -- INVALID ERROR NUMBER OR LEVEL$$ '// 'JOB ABORT DUE TO FATAL ERROR.', 72)            'JOB ABORT DUE TO FATAL ERROR.', 72)
 ...
 !      Formats.
 !
 9000 FORMAT ('0          ERROR MESSAGE SUMMARY' / ' LIBRARY    SUBROUTINE MESSAGE START             NERR',         ' LIBRARY    SUBROUTINE MESSAGE START             NERR', '     LEVEL     COUNT')         '     LEVEL     COUNT')
 ...
 9030 FORMAT (1X)
      ...
      IF (I .LT. 1 .OR. I .GT. 5) CALL XERMSG ('SLATEC', 'D1MACH', 'I OUT OF BOUNDS', 1, 2)         'I OUT OF BOUNDS', 1, 2)
      !
      D1MACH = DMACH(I)
      RETURN
      !
