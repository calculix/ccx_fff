...
!
      ...
      do i=2,n
         ...
         elseif(textpart(i)(1:10).eq.'AMPLITUDE=') then
            ...
            enddo
            if(j.eq.0) then
               write(*,*) '*ERROR reading *TEMPERATURE: nonexistent amplitude'                 '*ERROR reading *TEMPERATURE: nonexistent amplitude'
               write(*,*) '  '
               call inputerror(inpc,ipoinpc,iline, "*TEMPERATURE%",ier)                    "*TEMPERATURE%",ier)
               return
            endif
            iamplitude=j
         elseif(textpart(i)(1:10).eq.'TIMEDELAY=') THEN
            if(idelay.ne.0) then
               write(*,*) '*ERROR reading *TEMPERATURE: the parameter TIME'                 '*ERROR reading *TEMPERATURE: the parameter TIME'
            ...
            if(iamplitude.eq.0) then
               write(*,*) '*ERROR reading *TEMPERATURE: time delay must be'                 '*ERROR reading *TEMPERATURE: time delay must be'
         ...
         elseif(textpart(i)(1:6).eq.'BSTEP=') then
            ...
            if(istat.gt.0) call inputerror(inpc,ipoinpc,iline, "*TEMPERATURE%",ier)      "*TEMPERATURE%",ier)
            utempusesteps=.true.
         ...
         elseif(textpart(i)(1:8).eq.'DATASET=') then
            ...
            write(*,*) '*WARNING reading *TEMPERATURE: parameter not recognized:'              '*WARNING reading *TEMPERATURE: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*TEMPERATURE%")      "*TEMPERATURE%")
         endif
      enddo
      !
      ...
      if(user.and.(iamplitude.ne.0)) then
         write(*,*) '*WARNING reading *TEMPERATURE: no amplitude definition is'           '*WARNING reading *TEMPERATURE: no amplitude definition is'
