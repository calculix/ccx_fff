 ...
 50      CONTINUE
      ...
      IF( LOCIW.GT.LENIW ) THEN
         ...
         CALL XERMSG ('SLATEC', 'DCHKW', 'In ' // XERNAM // ', INTEGER work array too short.  ' //            'In ' // XERNAM // ', INTEGER work array too short.  ' // 'IWORK needs ' // XERN1 // '; have allocated ' // XERN2,            'IWORK needs ' // XERN1 // '; have allocated ' // XERN2,&
            1, 1)
      ENDIF
      !
      ...
      IF( LOCW.GT.LENW ) THEN
         ...
         WRITE (XERN1, '(I8)') LOCW
         WRITE (XERN2, '(I8)') LENW
         CALL XERMSG ('SLATEC', 'DCHKW', 'In ' // XERNAM // ', DOUBLE PRECISION work array too ' //            'In ' // XERNAM // ', DOUBLE PRECISION work array too ' // 'short.  RWORK needs ' // XERN1 // '; have allocated ' //            'short.  RWORK needs ' // XERN1 // '; have allocated ' //&
            XERN2, 1, 1)
      ENDIF
      RETURN
 ...
 430  CONTINUE
      ...
      NN = N
      IF (NN.LT.1) THEN
         CALL XERMSG ('SLATEC', 'QS2I1D', 'The number of values to be sorted was not positive.', 1, 1)            'The number of values to be sorted was not positive.', 1, 1)
         RETURN
      ENDIF
      IF( N.EQ.1 ) RETURN
      KK = ABS(KFLAG)
      IF ( KK.NE.1 ) THEN
         CALL XERMSG ('SLATEC', 'QS2I1D', 'The sort control parameter, K, was not 1 or -1.', 2, 1)            'The sort control parameter, K, was not 1 or -1.', 2, 1)
         RETURN
      ENDIF
      !
