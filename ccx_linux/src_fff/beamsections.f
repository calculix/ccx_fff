...
!
      ...
      if((istep.gt.0).and.(irstrt(1).ge.0)) then
         write(*,*) '*ERROR reading *BEAM SECTION: *BEAM SECTION should'             '*ERROR reading *BEAM SECTION: *BEAM SECTION should'
      ...
      do i=2,n
         ...
         elseif(textpart(i)(1:8).eq.'SECTION=') then
            ...
            elseif(textpart(i)(9:12).eq.'RECT') then
               ...
               write(*,*) '*ERROR reading *BEAM SECTION: unknown section'                 '*ERROR reading *BEAM SECTION: unknown section'
               ier=1
               return
            endif
         elseif(textpart(i)(1:8).eq.'OFFSET1=') then
            ...
            if(istat.gt.0) then
               call inputerror(inpc,ipoinpc,iline, "*BEAM SECTION%",ier)                    "*BEAM SECTION%",ier)
               return
            endif
         elseif(textpart(i)(1:8).eq.'OFFSET2=') then
            ...
            write(*,*) '*WARNING reading *BEAM SECTION: parameter not recognized:'             '*WARNING reading *BEAM SECTION: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*BEAM SECTION%")      "*BEAM SECTION%")
         endif
      enddo
      !
      ...
      !          iorientation=0
         ...
         enddo
         if(i.gt.norien) then
            write(*,*) '*ERROR reading *BEAM SECTION: nonexistent orientation'               '*ERROR reading *BEAM SECTION: nonexistent orientation'
            write(*,*) '  '
            call inputerror(inpc,ipoinpc,iline, "*BEAM SECTION%",ier)                 "*BEAM SECTION%",ier)
            return
         endif
      ...
      do j=istartset(i),iendset(i)
         if(ialset(j).gt.0) then
            if(lakon(ialset(j))(1:1).ne.'B') then
               write(*,*) '*ERROR reading *BEAM SECTION: *BEAM SECTION can'                 '*ERROR reading *BEAM SECTION: *BEAM SECTION can'
      ...
      if(.not.nodalthickness) then
         ...
         if(istat.gt.0) then
            write(*,*) '*ERROR reading *BEAM SECTION: first beam thickness is lacking'         '*ERROR reading *BEAM SECTION: first beam thickness is lacking'
            call inputerror(inpc,ipoinpc,iline, "*BEAM SECTION%",ier)              "*BEAM SECTION%",ier)
            return
         endif
         if(n.gt.1) then
            ...
            if(istat.gt.0) then
               write(*,*) '*ERROR reading *BEAM SECTION: ',                    '*ERROR reading *BEAM SECTION: ', 'second beam thickness is lacking'                    'second beam thickness is lacking'
               call inputerror(inpc,ipoinpc,iline, "*BEAM SECTION%",ier)                    "*BEAM SECTION%",ier)
               return
            endif
         else
      ...
      if(dd.lt.1.d-10) then
         write(*,*) '*ERROR reading *BEAM SECTION: normal in direction 1'             '*ERROR reading *BEAM SECTION: normal in direction 1'
         write(*,*) '       has zero size'
         ier=1
         return
      endif
