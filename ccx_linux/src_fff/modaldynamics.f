...
!
      ...
      !
      if(istep.lt.1) then
         write(*,*) '*ERROR reading *MODAL DYNAMIC: *MODAL DYNAMIC can only'            '*ERROR reading *MODAL DYNAMIC: *MODAL DYNAMIC can only'
      ...
      do i=2,n
         ...
         elseif(textpart(i)(1:11).eq.'STEADYSTATE') then
            ...
            write(*,*) '*WARNING reading *MODAL DYNAMIC: parameter not recognized:'            '*WARNING reading *MODAL DYNAMIC: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*MODAL DYNAMIC%")      "*MODAL DYNAMIC%")
         endif
      enddo
      !
      ...
      elseif(solver(1:16).eq.'ITERATIVESCALING') then
         write(*,*) '*WARNING reading *MODAL DYNAMIC: the iterative scaling'            '*WARNING reading *MODAL DYNAMIC: the iterative scaling'
      ...
      if((istat.lt.0).or.(key.eq.1)) then
         write(*,*) '*ERROR reading *MODAL DYNAMIC: definition not complete'             '*ERROR reading *MODAL DYNAMIC: definition not complete'
         write(*,*) '       '
         call inputerror(inpc,ipoinpc,iline, "*MODAL DYNAMIC%",ier)              "*MODAL DYNAMIC%",ier)
         return
      endif
      ...
      if(steadystate) then
         ...
         if(tinc.le.0.d0) then
            write(*,*) '*ERROR reading *MODAL DYNAMIC: initial increment'               '*ERROR reading *MODAL DYNAMIC: initial increment'
         ...
         if(tper.lt.0.d0) then
            write(*,*) '*ERROR reading *MODAL DYNAMIC: step size is negative'              '*ERROR reading *MODAL DYNAMIC: step size is negative'
            ier=1
            return
         ...
         if(tinc.lt.0.d0) then
            write(*,*) '*ERROR reading *MODAL DYNAMIC: initial increment size                  '*ERROR reading *MODAL DYNAMIC: initial increment size&
      is negative'
            ier=1
            return
