...
!
      ...
      do i=2,n
         if(textpart(i)(1:7).eq.'SOLVER=') then
            ...
            write(*,*) '*WARNING reading *BUCKLE: parameter not recognized:'              '*WARNING reading *BUCKLE: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*BUCKLE%")      "*BUCKLE%")
         endif
      enddo
      !
      ...
      if((istat.lt.0).or.(key.eq.1)) then
         ...
         write(*,*) '  '
         call inputerror(inpc,ipoinpc,iline, "*BUCKLE%",ier)              "*BUCKLE%",ier)
         return
      endif
