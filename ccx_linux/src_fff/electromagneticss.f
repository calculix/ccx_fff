...
!
      ...
      elseif((iperturb(1).eq.1).and.(istep.gt.1)) then
         write(*,*) '*ERROR reading *ELECTROMAGNETICS: perturbation analysis is'        '*ERROR reading *ELECTROMAGNETICS: perturbation analysis is'
      ...
      !
      if(istep.lt.1) then
         write(*,*) '*ERROR reading *ELECTROMAGNETICS: *ELECTROMAGNETICS can only'          '*ERROR reading *ELECTROMAGNETICS: *ELECTROMAGNETICS can only'
      ...
      do i=2,n
         ...
         elseif(textpart(i)(1:6).eq.'OMEGA=') then
            ...
            write(*,*) '*WARNING reading *ELECTROMAGNETICS: parameter not recognized:'         '*WARNING reading *ELECTROMAGNETICS: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*ELECTROMAGNETICS%")      "*ELECTROMAGNETICS%")
         endif
      enddo
      ...
      if(nmethod.eq.9) then
         if(iheat.eq.1) then
            if(ithermal(1).eq.0) then
               write(*,*) '*ERROR reading *ELECTROMAGNETICS: please define initial '              '*ERROR reading *ELECTROMAGNETICS: please define initial '
      ...
      if((nmethod.ne.10).and.(iperturb(1).ne.0)) then
         ...
         elseif(solver(1:6).eq.'PASTIX') then
            ...
            write(*,*) '*WARNING reading *ELECTROMAGNETICS: unknown solver;'                '*WARNING reading *ELECTROMAGNETICS: unknown solver;'
      ...
      ecified'
         ...
         if(istat.gt.0) then
            call inputerror(inpc,ipoinpc,iline, "*ELECTROMAGNETICS%",ier)                 "*ELECTROMAGNETICS%",ier)
            return
         endif
      ...
      ent size is negative'
         ...
         if(tper.le.0.d0) then
            write(*,*) '*ERROR reading *ELECTROMAGNETICS: step size is negative'             '*ERROR reading *ELECTROMAGNETICS: step size is negative'
         endif
      ...
      elseif(nmethod.eq.10) then
         ...
         write(*,*) '      for a steady state electromagnetics calculation'           '      for a steady state electromagnetics calculation'
