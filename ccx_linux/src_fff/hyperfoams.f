...
!
      ...
      r N=3 are allowed; '
               call inputerror(inpc,ipoinpc,iline, "*HYPERFOAM%",ier)                    "*HYPERFOAM%",ier)
               return
            endif
         else
            write(*,*) '*WARNING reading *HYPERFOAM: parameter not recognized:'              '*WARNING reading *HYPERFOAM: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*HYPERFOAM%")      "*HYPERFOAM%")
         endif
      enddo
      !
      nelcon(1,nmat)=ityp
      ...
      if(ityp.ne.-17) then
         ...
         do
            ...
            if((istat.lt.0).or.(key.eq.1)) then
               write(*,*) '*ERROR reading *HYPERFOAM: orthotropic definition'                 '*ERROR reading *HYPERFOAM: orthotropic definition'
