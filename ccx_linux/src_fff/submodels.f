...
!
      ...
      !
      if(istep.gt.0) then
         write(*,*) '*ERROR reading *SUBMODEL: *SURFACE should be placed'                '*ERROR reading *SUBMODEL: *SURFACE should be placed'
      ...
      if(nam.eq.0) then
         ...
         if(namtot_.lt.2) then
            write(*,*) '*ERROR reading *SUBMODEL: increase namtot_'                 '*ERROR reading *SUBMODEL: increase namtot_'
            ier=1
            return
         endif
         amta(1,1)=0.d0
      ...
      do i=2,n
         if(textpart(i)(1:5).eq.'TYPE=') then
            ...
            elseif(textpart(i)(6:9).eq.'NODE') then
               ...
               write(*,*) '*ERROR reading *SUBMODEL: unknown type'                '*ERROR reading *SUBMODEL: unknown type'
               ier=1
               return
            endif
         elseif(textpart(i)(1:12).eq.'GLOBALELSET=') then
            ...
            if(iglobset.gt.nset) then
               write(*,*) '*ERROR reading *SUBMODEL: global element set ',                  '*ERROR reading *SUBMODEL: global element set ',&
                  globset(1:ipos-1)
         ...
         elseif(textpart(i)(1:6).eq.'INPUT=') then
            ...
            write(*,*) '*WARNING reading *SUBMODEL: parameter not recognized:'              '*WARNING reading *SUBMODEL: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*SUBMODEL%")      "*SUBMODEL%")
         endif
      enddo
      !
      ...
      !          write(*,*) '*ERROR reading *SUBMODEL: no global elset'
      !          write(*,*) '       defined'
      !          ier=1
      !          return
      !       endif
      ...
      !       ipos=index(submset,' ')
      !       if(ipos.eq.1) then
      !          write(*,*) '*ERROR reading *SUBMODEL: no name specified'
      !          ier=1
      !          return
      !       endif
      submset(12:12)=type
         ...
         do
            ...
            do l=1,n
               if(nalset+1.gt.nalset_) then
                  write(*,*) '*ERROR reading *SUBMODEL: increase nalset_'                     '*ERROR reading *SUBMODEL: increase nalset_'
                  ier=1
                  return
               endif
               !
            ...
            do i=1,nset
               if(set(i).eq.facialset) then
                  do j=istartset(i),iendset(i)
                     nalset=nalset+1
                     if(nalset.gt.nalset_) then
                        write(*,*) '*ERROR in reading *SUBMODEL: increase nalset_'                         '*ERROR in reading *SUBMODEL: increase nalset_'
                        ier=1
                        return
                     endif
                     ialset(nalset)=ialset(j)
