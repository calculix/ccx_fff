...
!
      ...
      if((istep.gt.0).and.(irstrt(1).ge.0)) then
         write(*,*) '*ERROR reading *EXPANSION: *EXPANSION should be placed'             '*ERROR reading *EXPANSION: *EXPANSION should be placed'
      ...
      !
      if(nmat.eq.0) then
         write(*,*) '*ERROR reading *EXPANSION: *EXPANSION should be preceded'           '*ERROR reading *EXPANSION: *EXPANSION should be preceded'
      ...
      do i=2,n
         ...
         elseif(textpart(i)(1:5).eq.'ZERO=') then
            ...
            if(istat.gt.0) then
               call inputerror(inpc,ipoinpc,iline, "*EXPANSION%",ier)                    "*EXPANSION%",ier)
               return
            endif
         else
            write(*,*) '*WARNING reading *EXPANSION: parameter not recognized:'              '*WARNING reading *EXPANSION: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*EXPANSION%")      "*EXPANSION%")
         endif
      enddo
      !
      nalcon(1,nmat)=ityp
