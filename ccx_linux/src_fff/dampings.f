...
!
      ...
      !
      if(nmat.eq.0) then
         write(*,*) '*ERROR reading *DAMPING: *DAMPING should be preceded'             '*ERROR reading *DAMPING: *DAMPING should be preceded'
      ...
      do i=2,n
         if(textpart(i)(1:6).eq.'ALPHA=') then
            ...
            if(istat.gt.0) then
               call inputerror(inpc,ipoinpc,iline, "*DAMPING%",ier)                    "*DAMPING%",ier)
               return
            endif
         ...
         elseif(textpart(i)(1:11).eq.'STRUCTURAL=') then
            ...
            write(*,*) '*WARNING reading *DAMPING: parameter not recognized:'              '*WARNING reading *DAMPING: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*DAMPING%")      "*DAMPING%")
         endif
      enddo
      !
