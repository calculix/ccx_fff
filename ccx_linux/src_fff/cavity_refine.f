...
!
      ...
      do
        ...
        do j=1,nfcav
          ...
          ichange=1
          !      write(*,*) '*WARNING: CAVITY NOT CONVEX'
          !
          !     remove ielement
          !
          call nident(iecav,ielement,necav,id)
          ...
          do l=1,4
            ...
            if(id.ne.0) then
              if(ifcav(id).eq.iface) then
                ...
                ifcav(nfcav)=0
                nfcav=nfcav-1
                !      write(*,*) 'cavity subtract2',ibase,nfcav
                cycle
              endif
            endif
            !
      ...
      do i=1,nfcav
        ...
        if((volume.lt.hh**3/6000.d0).or.&
          ...
          irevert=1
          nktet=nktet-1
          !           write(*,*) '*INFO in cavity_refine: bad element'
        ...
        if(id.gt.0) then
          if(iecav(id).eq.ibasis) then
            ...
            if(irevert.eq.0) then
              ...
              if(isol.eq.0) then
                !                 write(*,*)
                !      &               '*ERROR in cavity_refine: node belongs to'
                !                 write(*,*) '       zero elements'
                ibasenewnodes(j)=0
              endif
            else
