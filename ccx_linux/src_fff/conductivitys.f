...
!
      ...
      if((istep.gt.0).and.(irstrt(1).ge.0)) then
         write(*,*) '*ERROR reading *CONDUCTIVITY: *CONDUCTIVITY should be'           '*ERROR reading *CONDUCTIVITY: *CONDUCTIVITY should be'
      ...
      do i=2,n
         if(textpart(i)(1:5).eq.'TYPE=') then
            ...
            write(*,*) '*WARNING reading *CONDUCTIVITY: parameter not recognized:'             '*WARNING reading *CONDUCTIVITY: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*CONDUCTIVITY%")      "*CONDUCTIVITY%")
         endif
      enddo
      !
      ncocon(1,nmat)=ityp
      ...
      if(ityp.eq.1) then
         do
            ...
            if(ntmat.gt.ntmat_) then
               write(*,*) '*ERROR reading *CONDUCTIVITY: increase ntmat_'                '*ERROR reading *CONDUCTIVITY: increase ntmat_'
               ier=1
               return
            endif
            do i=1,1
               read(textpart(i)(1:20),'(f20.0)',iostat=istat)&
                       cocon(i,ntmat,nmat)
               if(istat.gt.0) then
                  call inputerror(inpc,ipoinpc,iline, "*CONDUCTIVITY%",ier)                       "*CONDUCTIVITY%",ier)
                  return
               endif
            enddo
