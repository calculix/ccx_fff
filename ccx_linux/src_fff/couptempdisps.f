...
!
      ...
      do i=2,n
         if(textpart(i)(1:6).eq.'ALPHA=') then
            ...
            if(istat.gt.0) then
               call inputerror(inpc,ipoinpc,iline, "*COUPLED TEMPERATURE-DISPLACEMENT%",ier)                    "*COUPLED TEMPERATURE-DISPLACEMENT%",ier)
               return
            endif
         ...
         elseif(textpart(i)(1:17).eq.'TOTALTIMEATSTART=') then
            ...
            write(*,*) '*WARNING reading *COUPLED TEMPERATURE-DISPLACEMENT:'              '*WARNING reading *COUPLED TEMPERATURE-DISPLACEMENT:'
            write(*,*) '         parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*COUPLED TEMPERATURE-DISPLACEMENT%")      "*COUPLED TEMPERATURE-DISPLACEMENT%")
         endif
      enddo
      ...
      if((istat.lt.0).or.(key.eq.1)) then
         if(iperturb(1).ge.2) then
            write(*,*) '*WARNING reading *COUPLED TEMPERATURE-DISPLACEMENT:'                '*WARNING reading *COUPLED TEMPERATURE-DISPLACEMENT:'
            write(*,*) '         a nonlinear geometricanalysis is requested'                '         a nonlinear geometricanalysis is requested'
