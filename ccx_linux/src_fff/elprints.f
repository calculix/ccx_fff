...
!
      ...
      do ii=2,n
        ...
        elseif(textpart(ii)(1:10).eq.'FREQUENCY=') then
           ...
           if(istat.gt.0) then
              call inputerror(inpc,ipoinpc,iline, "*EL PRINT%",ier)                   "*EL PRINT%",ier)
              return
           endif
           if(joutl.eq.0) then
              do
        ...
        elseif(textpart(ii)(1:11).eq.'TIMEPOINTS=') then
           ...
           jout(2)=1
            ...
            write(*,*) '*WARNING reading *EL PRINT: parameter not recognized:'              '*WARNING reading *EL PRINT: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(ii)(1:index(textpart(ii),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*EL PRINT%")      "*EL PRINT%")
        endif
      enddo
      !
         ...
         do ii=1,n
            if(textpart(ii)(1:4).eq.'PEEQ') then
               if((nmethod.eq.2).or.(nmethod.eq.3)) then
                  write(*,*) '*WARNING reading *EL PRINT: selection of PEEQ'                     '*WARNING reading *EL PRINT: selection of PEEQ'
            ...
            elseif((textpart(ii)(1:4).eq.'CEEQ').or.&
               ...
               if((nmethod.eq.2).or.(nmethod.eq.3)) then
                  write(*,*) '*WARNING reading *EL PRINT: selection of CEEQ or CE or PE'             '*WARNING reading *EL PRINT: selection of CEEQ or CE or PE'
                  write(*,*) '         does not make sense for a'
                  write(*,*) '         frequency or bucking calculation'
                  cycle
               endif
               ...
               write(*,*) '*WARNING reading *EL PRINT: selection of CEEQ or CE or PE'             '*WARNING reading *EL PRINT: selection of CEEQ or CE or PE'
               write(*,*) '         is converted into PEEQ; no distinction'                  '         is converted into PEEQ; no distinction'
               write(*,*) '        is made between PEEQ, CEEQ, CE and PE'                '        is made between PEEQ, CEEQ, CE and PE'
            elseif(textpart(ii)(1:3).eq.'SDV') then
               if((nmethod.eq.2).or.(nmethod.eq.3)) then
                  write(*,*) '*WARNING reading *EL PRINT: selection of SDV'                    '*WARNING reading *EL PRINT: selection of SDV'
                  write(*,*) '         does not make sense for a'
                  write(*,*) '         frequency or bucking calculation'
                  cycle
               endif
            ...
            elseif(textpart(ii)(1:4).eq.'HFL ') then
               if(ithermal(1).lt.2) then
                  write(*,*) '*WARNING reading *EL PRINT: HFL only makes '                     '*WARNING reading *EL PRINT: HFL only makes '
            ...
            elseif((textpart(ii)(1:4).eq.'SVF ').or.&
               ...
               if(nef.eq.0) then
                  write(*,*) '*WARNING reading *EL PRINT: SVF or HFLF only'                     '*WARNING reading *EL PRINT: SVF or HFLF only'
            ...
            elseif((textpart(ii)(1:4).ne.'S   ').and.&
               ...
               write(*,*) '*WARNING reading *EL PRINT: label not applicable'                   '*WARNING reading *EL PRINT: label not applicable'
               write(*,*) '         or unknown; '
               call inputerror(inpc,ipoinpc,iline, "*EL PRINT%",ier)                    "*EL PRINT%",ier)
               return
            endif
