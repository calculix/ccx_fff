...
!
      ...
      !
      !      write(*,*) 'initialcfdfem element height'
      !
      dhelmax=0.d0
      do nelem=1,ne
      ...
      !      !
      !      enddo loop
      !      write(*,*) 'initialcfdfem node height'
      loop:do i=1,nk
      index=iponoel(i)
      if(index.le.0) cycle
      !      write(*,*) 'old value ',i,dh(i)
      dh(i)=1.d30
      !
      !     loop over all elements belonging to the edge node:
      !     determining the minimum height
      ...
      !         write(*,*) 'initialcfdfem',nelem,dhel(nelem)
      enddo
      !
      ...
      do node=1,nk
        ...
        !      endif
        !      start shallow
        !      rho=dsqrt(vold(4,node)/5.d0+(0.005*co(1,node))**2)
        !      vcon(0,node)=rho*(cp*(temp-physcon(1))+
        !      &           (vold(1,node)**2+vold(2,node)**2+vold(3,node)**2)
        !      &           /2.d0)
        !      end shallow
