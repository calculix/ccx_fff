...
!
      ...
      if(iturbulent.ne.0) then
         do i=1,nk
            if(inomat(i).eq.0) cycle
            !             write(*,*) 'cfdconv.f ',i,vtu(1,i),vtu(2,i)
            do j=1,2
      ...
      if(iturbulent.eq.0) then
         ...
         if((compressible.eq.1).and.(shockcoef>0)) then
            ...
            if(iit.gt.1000) then
               ...
               enddo
               !                write(*,*) 'cfdconv ',iit,bmax
               !                if(bmax.lt.1.d-4) convergence=1
      ...
      !       endif
      !       if(iturbulent.ne.0) then
      !          if(dabs(vconmax(5)).gt.1.d-3) then
      !             factor=min(factor,vconmax(5)/vmax(5)*0.001)
      !          endif
      !          if(dabs(vconmax(6)).gt.1.d-3) then
      !             factor=min(factor,vconmax(6)/vmax(6)*0.001)
      !          endif
      !       endif
      !
      return
