      ...
      BRANCH JOINT IDELCHIK2'
        ...
        elseif((lakon(nelem)(2:3).eq.'OR').and.&
          ...
          if(prop(index+3).lt.0.d0) then
            write(*,*) '*ERROR in checkinputvaluesnet:'
            write(*,*) '        length of the orifice is ', 'not positive'                 'not positive'
            write(*,*) '       element number: ',nelem
            call exit(201)
          endif
          if((lakon(nelem)(2:5).ne.'ORC1').and.&
            ...
            if((prop(index+4).gt.1.d-20).and.&
              ...
              write(*,*) '*ERROR in checkinputvaluesnet:'                   '*ERROR in checkinputvaluesnet:'
              write(*,*) 'either the radius of ', 'the orifice must be zero or the chamfer angle'                   'the orifice must be zero or the chamfer angle'
              write(*,*) '       element number: ',nelem
              call exit(201)
            endif
          endif
          if(prop(index+4)/prop(index+2).lt.0.d0) then
            write(*,*) '*ERROR in checkinputvaluesnet: '
            write(*,*) '        r/d of an orifice must not be negative'                 '        r/d of an orifice must not be negative'
            write(*,*) '       element number: ',nelem
            call exit(201)
          endif
          if(prop(index+4)/prop(index+2).gt.0.82d0) then
            write(*,*) '*WARNING in checkinputvaluesnet: '                 '*WARNING in checkinputvaluesnet: '
            write(*,*) '        r/d of an orifice ', 'must not exceed 0.82'                 'must not exceed 0.82'
            write(*,*) '         element number: ',nelem
          endif
          if(prop(index+5).lt.0.d0) then
            write(*,*) '*ERROR in checkinputvaluesnet:'
            write(*,*) '       the chamfer angle of an ', 'orifice must not be negative'                 'orifice must not be negative'
            write(*,*) '       element number: ',nelem
            call exit(201)
          endif
          if(prop(index+5).gt.90.d0) then
            write(*,*) '*ERROR in checkinputvaluesnet: '                 '*ERROR in checkinputvaluesnet: '
            write(*,*) '       the chamfer angle of an ', 'orifice must not exceed 90°'                 'orifice must not exceed 90°'
            write(*,*) '       element number: ',nelem
            call exit(201)
          endif
          ...
          if(prop(index+3)/prop(index+2).lt.0.d0) then
            write(*,*) '*ERROR in checkinputvaluesnet:'                 '*ERROR in checkinputvaluesnet:'
            write(*,*) '        L/d of an orifice ', 'must not be negative'                 'must not be negative'
            write(*,*) '       element number: ',nelem
            call exit(201)
          endif
          if(lakon(nelem)(4:4).eq.'P') then
            if(prop(index+3)/prop(index+2).gt.2.d0) then
              write(*,*) '*WARNING in checkinputvaluesnet: '                   '*WARNING in checkinputvaluesnet: '
              write(*,*) '       L/d of an orifice ', 'with Parker must not exceed 2'                   'with Parker must not exceed 2'
              write(*,*) '       element number: ',nelem
            !     call exit(201)
            endif
          endif
        elseif((lakon(nelem)(2:5).eq.'ORBT'))then
          if(prop(index+2).lt.0.d0) then
            write(*,*) '*ERROR in checkinputvaluesnet:'
            write(*,*) '        ps1/pt1 (bleedtapping) ', 'must not be negative'                 'must not be negative'
            write(*,*) '       element number: ',nelem
            call exit(201)
          endif
          if(prop(index+2).gt.1.d0) then
            write(*,*) '*ERROR in checkinputvaluesnet: '
            write(*,*) '       ps1/pt1 (bleed tapping) ', 'must not exceed 1'                 'must not exceed 1'
            write(*,*) '       element number: ',nelem
            call exit(201)
          endif
        elseif((lakon(nelem)(2:5).eq.'ORPN'))then
          if(prop(index+2).lt.0.d0) then
            write(*,*) '*ERROR in checkinputvaluesnet: '
            write(*,*) '       theta (preswirlnozzle) ', 'must not be negative'                 'must not be negative'
            write(*,*) '       element number: ',nelem
            call exit(201)
          endif
          if(prop(index+2).gt.90.d0) then
            write(*,*) '*ERROR in checkinputvaluesnet:'
            write(*,*) '       theta (preswirl nozzle) ', 'must not exceed 90°'                 'must not exceed 90°'
            write(*,*) '       element number: ',nelem
            call exit(201)
          endif
          if(prop(index+3).lt.0.d0) then
            write(*,*) '*ERROR in checkinputvaluesnet: '
            write(*,*) '       k_phi (preswirl nozzle) ', 'must not be negative'                 'must not be negative'
            write(*,*) '       element number: ',nelem
            call exit(201)
          endif
          if(prop(index+3).gt.1.05d0) then
            write(*,*) '*ERROR in checkinputvaluesnet: '
            write(*,*) '       k_phi (preswirlnozzle) ', 'must not exceed 1.05'                 'must not exceed 1.05'
            write(*,*) '       element number: ',nelem
            call exit(201)
          endif
        ...
        elseif((lakon(nelem)(2:4).eq.'LAB').and.&
          ...
          if((prop(index+1).gt.1000.d0)&
            ...
            write(*,*) '*ERROR in checkinputvaluesnet: ',                 '*ERROR in checkinputvaluesnet: ', 'the selected pitch t'                 'the selected pitch t'
          ...
          if((prop(index+2).gt.100.d0)&
            ...
            write(*,*) '*ERROR in checkinputvaluesnet: ',                 '*ERROR in checkinputvaluesnet: ', 'the selected gap s'                 'the selected gap s'
            write(*,*) '       for the labyrinth is not correct'
            write(*,*) '       0<=s<=100 mm'
            write(*,*) '       element number: ',nelem
            call exit(201)
        ...
        elseif((lakon(nelem)(2:6).eq.'CARBS'))then
          if(lakon(nelem)(2:8).eq.'CARBSGE') then
            if(prop(index+1).le.0.d0) then
              write(*,*) '*ERROR in checkinputvaluesnet:'
              write(*,*) '       the selected diameter of ', 'the carbon seal'                   'the carbon seal'
              write(*,*) '       has been defined as less or equal to 0'                   '       has been defined as less or equal to 0'
              write(*,*) '       element number: ',nelem
              call exit(201)
            endif
          else
            if((prop(index+1).le.0.d0)&
              ...
              write(*,*) '       or the selected gap of the carbon seal'                   '       or the selected gap of the carbon seal'
              write(*,*) '       has been defined as less or equal to 0'                   '       has been defined as less or equal to 0'
              write(*,*) '       element number: ',nelem
              call exit(201)
            endif
        ...
        elseif((lakon(nelem)(2:4).eq.'RCVL'))then
          if(prop(index+2).lt.(prop(index+3))) then
            write(*,*) '*ERROR in checkinputvaluesnet: '
            write(*,*) '       element TYPE=ROTATING CAVITY ', '(Radial inflow)'                 '(Radial inflow)'
            write(*,*) '       the specified upstream radius is ', 'smaller than'                 'smaller than'
        ...
        elseif(lakon(nelem)(2:3).eq.'RE') then
          ...
          elseif((lakon(nelem)(2:5).eq.'REEL'))then
            if(prop(index+1).ge.(prop(index+2))) then
              write(*,*) '*ERROR in checkinputvaluesnet:'
              write(*,*) '       Section A1 is greater than section A2'                   '       Section A1 is greater than section A2'
              write(*,*) '       element number: ',nelem
              call exit(201)
            endif
          !
          elseif((lakon(nelem)(2:5).eq.'RECO'))then
            if(prop(index+1).lt.(prop(index+2))) then
              write(*,*) '*ERROR in checkinputvaluesnet:'
              write(*,*) '       Section A2 is greater than section A1'                   '       Section A2 is greater than section A1'
              write(*,*) '       element number: ',nelem
              call exit(201)
            endif
          endif
          ...
          if((lakon(nelem)(2:5).eq.'REBR'))then
            if((prop(index+1).le.0.d0)&
              ...
              write(*,*) '       all three elements must be ', 'different from 0'                   'different from 0'
              write(*,*) '       element number: ',nelem
              call exit(201)
            !
            ...
            elseif((prop(index+7).gt.90)&
              ...
              write(*,*) '*ERROR in checkinputvaluesnet:'
              write(*,*) '       trying to define a branch '
              write(*,*) '       alpha1 & 2 cannot greater than ', '90 gegrees'                   '90 gegrees'
              write(*,*) '       element number: ',nelem
              call exit(201)
            !
            elseif((lakon(nelem)(6:8).eq.'SI1')&
              ...
              if(prop(index+7).gt.0) then
                write(*,*) '*ERROR in checkinputvaluesnet:'
                write(*,*) '       trying to define a branch '
                write(*,*) '       Type IDELCHIK JOINT1 or SPLIT1&2'                     '       Type IDELCHIK JOINT1 or SPLIT1&2'
        ...
        elseif((lakon(nelem)(2:3).eq.'VO'))then
          ...
          if((lakon(nelem)(2:5).eq.'VOFR'))then
            ...
            if(prop(index+6).ne.0d0) then
              ...
              if(prop(index+7).ne.0.d0) then
                ...
                write(*,*) '       rotation speed and upstream element'                     '       rotation speed and upstream element'
          ...
          elseif((lakon(nelem)(2:5).eq.'VOFO'))then
            ...
            if(prop(index+4).le.0.d0) then
              ...
              write(*,*)'       Core swirl ratio Kr is strictly ', 'positive'                   'positive'
              write(*,*) '       element number: ',nelem
              call exit(201)
            endif
            if(prop(index+4).gt.1.d0) then
              ...
              write(*,*)'       trying to define a FORCED VORTEX'
              write(*,*)'       Core swirl ratio Kr is ', 'greater than 1'                   'greater than 1'
              write(*,*) '       element number: ',nelem
            !     call exit(201)
            endif
            !
            ...
            if(prop(index+5).le.0.d0) then
              write(*,*)'*ERROR in checkinputvaluesnet:'
              write(*,*)'       trying to define a FORCED VORTEX'
              write(*,*) '       Rotation speed n is strictly positive'                   '       Rotation speed n is strictly positive'
              write(*,*) '       element number: ',nelem
              call exit(201)
            endif
          endif
        ...
        elseif((lakon(nelem)(2:4).eq.'ATR').or.&
          ...
          elseif(prop(index+3).ne.0.d0) then
            if(prop(index+2).ne.0.d0) then
              ...
              write(*,*)'       but tangential velocity ', 'is already defined'                   'is already defined'
              write(*,*)'       element number: ',nelem
              call exit(201)
            endif
          endif
