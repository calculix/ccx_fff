...
!
      ...
      !
      if(istep.gt.0) then
        write(*,*) '*ERROR reading *INITIAL CONDITIONS: *INITIAL CONDITIONS'             '*ERROR reading *INITIAL CONDITIONS: *INITIAL CONDITIONS'
      ...
      do ij=2,n
        if(textpart(ij)(1:16).eq.'TYPE=TEMPERATURE') then
          ...
          do
            ...
            read(textpart(2)(1:20),'(f20.0)',iostat=istat)&
                 temperature
            if(istat.gt.0) then
              call inputerror(inpc,ipoinpc,iline, "*INITIAL CONDITIONS%",ier)                   "*INITIAL CONDITIONS%",ier)
              return
            endif
            ...
            if(istat.eq.0) then
              if(l.gt.nk) then
                write(*,*) '*WARNING reading *INITIAL CONDITIONS: node ',l                     '*WARNING reading *INITIAL CONDITIONS: node ',l
                write(*,*)'          exceeds the largest defined ', 'node number'                     'node number'
                cycle
              endif
              t0(l)=temperature
              t1(l)=temperature
              ...
              if(ii.gt.nset) then
                noset(ipos:ipos)=' '
                write(*,*) '*ERROR reading *INITIAL CONDITIONS: node set '                     '*ERROR reading *INITIAL CONDITIONS: node set '&
                     ,noset
        ...
        elseif(textpart(ij)(1:11).eq.'TYPE=STRESS') then
          ...
          do
            ...
            if(l.gt.ne) then
              write(*,*) '*WARNING reading *INITIAL CONDITIONS: element ',l                   '*WARNING reading *INITIAL CONDITIONS: element ',l
              write(*,*)'          exceeds the largest defined ', 'element number'                   'element number'
              cycle
            endif
        ...
        elseif(textpart(ij)(1:13).eq.'TYPE=SOLUTION') then
          ...
          !      if(.not.user) then
          !      write(*,*)
          !      &            '*ERROR reading *INITIAL CONDITIONS: TYPE=SOLUTION'
        ...
        !
          ...
          write(*,*) '*WARNING reading *INITIAL CONDITIONS: parameter not recognized:'      '*WARNING reading *INITIAL CONDITIONS: parameter not recognized:'
          write(*,*) '         ',&
               textpart(ij)(1:index(textpart(ij),' ')-1)
          call inputwarning(inpc,ipoinpc,iline, "*INITIAL CONDITIONS%")               "*INITIAL CONDITIONS%")
        endif
      enddo
      !
