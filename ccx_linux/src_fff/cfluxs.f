      ...
      do i=2,n
         ...
         elseif(textpart(i)(1:10).eq.'AMPLITUDE=') then
            ...
            if(j.eq.0) then
               ...
               write(*,*) '  '
               call inputerror(inpc,ipoinpc,iline, "*CFLUX%",ier)                    "*CFLUX%",ier)
               return
            endif
            iamplitude=j
         elseif(textpart(i)(1:10).eq.'TIMEDELAY=') THEN
            if(idelay.ne.0) then
               write(*,*) '*ERROR reading *CFLUX: the parameter TIME DELAY'                 '*ERROR reading *CFLUX: the parameter TIME DELAY'
         ...
         elseif(textpart(i)(1:3).eq.'ADD') then
            ...
            write(*,*) '*WARNING reading *CFLUX: parameter not recognized:'              '*WARNING reading *CFLUX: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*CFLUX%")      "*CFLUX%")
         endif
      enddo
      !
