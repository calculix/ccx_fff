...
!
      ...
      if((istep.gt.0).and.(irstrt(1).ge.0)) then
         write(*,*) '*ERROR reading *MEMBRANE SECTION: *MEMBRANE SECTION should'            '*ERROR reading *MEMBRANE SECTION: *MEMBRANE SECTION should'
      ...
      do i=2,n
         ...
         elseif(textpart(i)(1:7).eq.'OFFSET=') then
            ...
            if(istat.gt.0) then
               call inputerror(inpc,ipoinpc,iline, "*MEMBRANE SECTION%",ier)                    "*MEMBRANE SECTION%",ier)
               return
            endif
         elseif(textpart(i)(1:9).eq.'COMPOSITE') then
            composite=.true.
         else
            write(*,*) '*WARNING reading *MEMBRANE SECTION: parameter not recognized:'         '*WARNING reading *MEMBRANE SECTION: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*MEMBRANE SECTION%")      "*MEMBRANE SECTION%")
         endif
      enddo
      !
      ...
      if(.not.composite) then
         ...
         enddo
         if(i.gt.nmat) then
            write(*,*) '*ERROR reading *MEMBRANE SECTION: nonexistent material'              '*ERROR reading *MEMBRANE SECTION: nonexistent material'
            call inputerror(inpc,ipoinpc,iline, "*MEMBRANE SECTION%",ier)                 "*MEMBRANE SECTION%",ier)
            return
         endif
         imaterial=i
      ...
      !          iorientation=0
         ...
         enddo
         if(i.gt.norien) then
            write(*,*) '*ERROR reading *MEMBRANE SECTION: nonexistent orientation'             '*ERROR reading *MEMBRANE SECTION: nonexistent orientation'
            call inputerror(inpc,ipoinpc,iline, "*MEMBRANE SECTION%",ier)                 "*MEMBRANE SECTION%",ier)
            return
         endif
         iorientation=i
      ...
      if(.not.composite) then
         if(.not.nodalthickness) then
            ...
            if(istat.gt.0) then
               write(*,*) '*ERROR reading *MEMBRANE SECTION: membrane thickness is lacking'       '*ERROR reading *MEMBRANE SECTION: membrane thickness is lacking'
               call inputerror(inpc,ipoinpc,iline, "*MEMBRANE SECTION%",ier)                    "*MEMBRANE SECTION%",ier)
               return
            endif
            ...
            do j=istartset(iset),iendset(iset)
               if(ialset(j).gt.0) then
                  if(lakon(ialset(j))(1:1).ne.'M') then
                     write(*,*) '*ERROR reading *MEMBRANE SECTION: *MEMBRANE SECTION can'               '*ERROR reading *MEMBRANE SECTION: *MEMBRANE SECTION can'
                     write(*,*) '       only be used for membrane elements.'                       '       only be used for membrane elements.'
                     write(*,*) '       Element ',ialset(j), ' is not a membrane element.'                       ' is not a membrane element.'
                     ier=1
                     return
                  endif
                  ...
                  do
                     ...
                     if(lakon(k)(1:1).ne.'M') then
                        write(*,*) '*ERROR reading *MEMBRANE SECTION: *MEMBRANE SECTION can'               '*ERROR reading *MEMBRANE SECTION: *MEMBRANE SECTION can'
                        write(*,*) '       only be used for membrane elements.'                          '       only be used for membrane elements.'
                        write(*,*) '       Element ',k, ' is not a membrane element.'                          ' is not a membrane element.'
                        ier=1
                        return
                     endif
                     indexe=ipkon(k)
      ...
      !
         ...
         do
            ...
            if(istat.gt.0) then
               write(*,*) '*ERROR reading *MEMBRANE SECTION: no material defined'                 '*ERROR reading *MEMBRANE SECTION: no material defined'
               call inputerror(inpc,ipoinpc,iline, "*MEMBRANE SECTION%",ier)                    "*MEMBRANE SECTION%",ier)
               return
            endif
            !
            ...
            do j=istartset(iset),iendset(iset)
               if(ialset(j).gt.0) then
                  if((lakon(ialset(j))(1:3).ne.'M3D8R').and.&
                     ...
                     write(*,*) '*ERROR reading *MEMBRANE SECTION: *MEMBRANE SECTION'                 '*ERROR reading *MEMBRANE SECTION: *MEMBRANE SECTION'
                     write(*,*) '       with the option COMPOSITE can'                       '       with the option COMPOSITE can'
                     write(*,*) '       only be used for M3D8R or M3D6 membrane elements.'              '       only be used for M3D8R or M3D6 membrane elements.'
                     write(*,*) '       Element ',ialset(j), ' is not a M3D8R nor a M3D6 membrane element.'                       ' is not a M3D8R nor a M3D6 membrane element.'
                     ier=1
                     return
                  endif
                  indexe=ipkon(ialset(j))
                  ...
                  do
                     ...
                     if((lakon(k)(1:3).ne.'M3D8R').and.&
                        ...
                        write(*,*) '    with the option COMPOSITE can'                             '    with the option COMPOSITE can'
                        write(*,*) '       only be used for M3D8R or M3D6 membrane elements.'              '       only be used for M3D8R or M3D6 membrane elements.'
                        write(*,*) '       Element ',k, ' is not a M3D8R nor a M3D6 membrane element.'                          ' is not a M3D8R nor a M3D6 membrane element.'
                        ier=1
                        return
                     endif
                     indexe=ipkon(k)
