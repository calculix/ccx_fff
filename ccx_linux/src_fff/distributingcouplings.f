...
!
      ...
      do i=2,n
         if(textpart(i)(1:6).eq.'ELSET=') then
            ...
            call inputwarning(inpc,ipoinpc,iline, "*DISTRIBUTING COUPLING%")      "*DISTRIBUTING COUPLING%")
         endif
      enddo
      !
      if(elset(1:1).eq.' ') then
         ...
         call inputerror(inpc,ipoinpc,iline, "*DISTRIBUTING COUPLING%",ier)              "*DISTRIBUTING COUPLING%",ier)
         return
      endif
      !
      ...
      if(i.gt.nset) then
         write(*,*) '*ERROR reading *DISTRIBUTING COUPLING:'
         write(*,*) '       element set ',elset(1:ipos-1), ' is not defined'               ' is not defined'
         ier=1
         return
      endif
      !
      ...
      if(istartset(i).ne.iendset(i)) then
         write(*,*) '*ERROR reading *DISTRIBUTING COUPLING:'
         write(*,*) '       element set ',elset(1:ipos-1), ' contains more than one element'              ' contains more than one element'
         ier=1
         return
      endif
      !
            ...
            if(newmpc.eq.0) then
               ...
               if(id.gt.0) then
                  if(ikmpc(id).eq.idof) then
                     write(*,*) '*ERROR reading *DISTRIBUTING COUPLING:'
                     write(*,*) '       dof 1 of node ',node, ' is already used'                          ' is already used'
                     ier=1
                     return
                  endif
               endif
            ...
            do j=istartset(i),iendset(i)
               if(ialset(j).gt.0) then
                  ...
                  if(newmpc.eq.0) then
                     ...
                     if(id.gt.0) then
                        if(ikmpc(id).eq.idof) then
                           write(*,*) '*ERROR reading *DISTRIBUTING COUPLING:'                             '*ERROR reading *DISTRIBUTING COUPLING:'
                           write(*,*) '       dof 1 of node ',node, ' is already used'                                ' is already used'
                           ier=1
                           return
                        endif
      ...
      do idir=2,3
         ...
         if(id.gt.0) then
            if(ikmpc(id).eq.idof) then
               write(*,*) '*ERROR reading *DISTRIBUTING COUPLING:'
               write(*,*) '       dof',idir,' of node ',node, ' is already used'                    ' is already used'
               ier=1
               return
            endif
         endif
