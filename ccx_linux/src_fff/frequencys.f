...
!
      ...
      !
      if(istep.lt.1) then
         write(*,*) '*ERROR reading *FREQUENCY: *FREQUENCY can only be used'            '*ERROR reading *FREQUENCY: *FREQUENCY can only be used'
         write(*,*) '  within a STEP'
         ier=1
         return
      endif
      ...
      do i=2,n
         ...
         elseif(textpart(i)(1:15).eq.'CYCMPC=INACTIVE') then
            ...
            write(*,*) '*WARNING reading *FREQUENCY: parameter not recognized:'              '*WARNING reading *FREQUENCY: parameter not recognized:'
            write(*,*) '         ',&
                       textpart(i)(1:index(textpart(i),' ')-1)
            call inputwarning(inpc,ipoinpc,iline, "*FREQUENCY%")      "*FREQUENCY%")
         endif
      enddo
      !
      ...
      if(isolver.ne.6) then
         ...
         if((istat.lt.0).or.(key.eq.1)) then
            write(*,*) '*ERROR reading *FREQUENCY: definition not complete'             '*ERROR reading *FREQUENCY: definition not complete'
            write(*,*) '  '
            call inputerror(inpc,ipoinpc,iline, "*FREQUENCY%",ier)                 "*FREQUENCY%",ier)
            return
         endif
